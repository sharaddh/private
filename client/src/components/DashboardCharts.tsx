  ...
  return (
    <div className="bg-th-surface rounded-xl p-3 sm:p-5 shadow-md h-full">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        <span
          className={`text-[12px] sm:text-[14px] font-bold px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg ${trendUp ? 'text-[#1ed760] bg-[#1ed760]/10' : 'text-[#e91429] bg-[#e91429]/10'}`}
        >
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -12 }}>
            ...
            <XAxis
              ...
              tick={{ fontSize: 12, fill: 'rgba(107,114,128,0.6)' }}
        ...
        <div className="h-28 w-28 sm:h-40 sm:w-40 flex-shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              >
                {data.map((d, i) => (
                  <Cell key={d.status || i} fill={DONUT_COLORS[i % DONUT_COLORS.length]} />
                ))}
              </Pie>
              <Tooltip
        ...
        <div className="flex-1 space-y-1.5 sm:space-y-2">
          {data.map((d, i) => (
            <div key={`${d.status}-${i}`} className="flex items-center gap-1.5 sm:gap-2">
              ...
              />
              <span className="text-[12px] sm:text-[14px] font-medium text-th-secondary flex-1 truncate">
                {d.status}
              </span>
              <span className="text-[12px] sm:text-[14px] font-bold text-th-text">{d.count}</span>
              <span className="text-[10px] sm:text-[12px] text-th-muted">
...
export function PaymentModeBarChart({
  data,
  dark,
}: {
...
export function SalesVsCollectionChart({
  salesData,
  collectionData,
  dark,
}: {
  ...
  return (
    <div className="bg-th-surface rounded-xl p-5 shadow-md h-full">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-[17px] font-bold text-th-text uppercase tracking-wider">
            Sales vs Collection
          </h3>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={merged} margin={{ top: 4, right: 4, bottom: 0, left: -12 }}>
            ...
            <XAxis
              dataKey="date"
              tickFormatter={formatDateLabel}
              tick={{ fontSize: 11, fill: 'rgba(107,114,128,0.6)' }}
              interval="preserveStartEnd"
              axisLine={false}
              tickLine={false}
            />
...
export function TodayPaymentDonut({
  data,
  dark,
}: {
  data: { mode: string; total: number; count: number }[];
            ...
            <PieChart>
              ...
              >
                {data.map((d, i) => (
                  <Cell
                    key={d.mode || i}
                    fill={PAYMENT_COLORS[d.mode] || DONUT_COLORS[i % DONUT_COLORS.length]}
                  />
                ))}
              </Pie>
              <Tooltip
          ...
          {data.map((d, i) => (
            <div key={`${d.mode}-${i}`} className="flex items-center gap-1.5 sm:gap-2">
              <span
                className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0"
                style={{
                  backgroundColor: PAYMENT_COLORS[d.mode] || DONUT_COLORS[i % DONUT_COLORS.length],
                }}
              />
              <span className="text-[12px] sm:text-[14px] font-medium text-th-secondary flex-1 truncate">
                {d.mode}
              </span>
              <span className="text-[12px] sm:text-[14px] font-bold text-th-text">
        ...
        <div className="h-40 w-40 flex-shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              >
                {data.map((d, i) => (
                  <Cell key={d.category || i} fill={DONUT_COLORS[i % DONUT_COLORS.length]} />
                ))}
              </Pie>
              <Tooltip
                formatter={(value, name) => [Number(value), 'Items']}
                contentStyle={tooltipStyle}
              />
            </PieChart>
          </ResponsiveContainer>
          ...
          {data.slice(0, 5).map((d, i) => (
            <div key={`${d.category}-${i}`} className="flex items-center gap-2">
              <span
                ...
                style={{ backgroundColor: DONUT_COLORS[i % DONUT_COLORS.length] }}
              />
              <span className="text-[14px] font-medium text-th-secondary flex-1 truncate">
                {d.category}
              </span>
