...
function DemandCell({ power, qty, need, demandQty, onToggle, onRemove }: DemandCellProps) {
  ...
  return (
    <div
      onClick={onToggle}
      className={`relative flex flex-col items-center gap-1.5 py-3 px-2 rounded-lg border cursor-pointer transition-all ${border}`}
    >
      {selected && (
        <button
          type="button"
      ...
      <div className="flex items-baseline gap-1.5">
        <span
          className={`text-lg sm:text-xl font-bold leading-none ${qty > 0 ? qtyClr : 'text-th-muted'}`}
        >
          {qty}
        </span>
        {selected ? (
...
export default function LensDemandGrid({
  item,
  lensType,
  demandTarget,
  demandSel,
  ...
  const quantities =
    lensType === 'plain'
      ? item.quantities?.sph || {}
      : item.quantities?.[lensType as LensType] || {};
                  ...
                  return (
                    <div key={cyl}>
                      ...
                      >
                        ...
                        <span className="px-2.5 py-0.5 rounded-pill bg-th-elevated text-th-muted text-body font-bold">
                          CYL {cyl === '+0.00' || cyl === '0.00' ? '0.00' : cyl}
                        </span>
