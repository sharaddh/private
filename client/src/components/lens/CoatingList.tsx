...
export default function CoatingList({
  ...
  const priceFields = (
    ...
    neg: string,
    pos: string,
    onChange: (neg: string, pos: string) => void,
    onEnter: () => void
  ) => (
    <div className="flex items-center gap-2">
      <label
        className={`flex-1 flex items-center gap-1.5 min-w-0 px-2 py-1.5 ${id === 'new' ? 'bg-th-surface' : 'bg-th-input'} border border-th-border rounded-lg focus-within:border-primary-500 transition-colors`}
      >
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between gap-2 mb-3">
        <h3 className="text-small font-bold text-th-text uppercase tracking-wider truncate">
          Coatings
        </h3>
        {!readonly && (
          <button
      ...
      <div className="flex-1 overflow-auto scrollbar-thin space-y-1.5 pr-0.5">
        {items.map((item) => {
          ...
          return (
            ...
            >
              <div
                className={`w-1 h-10 rounded-sm flex-shrink-0 transition-colors ${
              ...
              {isEditing ? (
                ...
                >
                  <div className="flex gap-1">
                    ...
                    <button
                      ...
                      className="p-2 text-primary-500 hover:bg-primary-500/10 rounded-lg transition-colors shrink-0"
                    >
                    ...
                    <button
                      ...
                      className="p-2 text-th-muted hover:bg-th-elevated rounded-lg transition-colors shrink-0"
                    >
                ...
                <>
                  <div className="flex-1 min-w-0">
                    ...
                    <div
                      className={`text-micro mt-0.5 font-medium ${
                        totalQty > 0 ? 'text-primary-500' : 'text-th-muted'
                      }`}
                    >
                      {totalQty > 0 ? `${totalQty} in stock` : 'Empty'}
                    </div>
                    <div className="text-micro mt-0.5 font-bold text-th-muted truncate whitespace-nowrap">
                    ...
                    >
                      <button
                        ...
                        onClick={() => {
                          ...
                          setEditName(item.coating);
                          setEditPriceNeg(String(item.priceNeg ?? item.price ?? 0));
                          setEditPricePos(String(item.pricePos ?? item.price ?? 0));
                        }}
