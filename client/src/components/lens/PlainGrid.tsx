...
export default function PlainGrid({
  quantities,
  onIncrement,
  onDecrement,
  onAddToCart,
  ...
  const cardBase =
    'flex flex-col items-center gap-1.5 py-3 px-2 rounded-lg border border-th-border bg-th-elevated';
  const label = (
    <span className="text-sm sm:text-base font-bold text-th-secondary leading-none">0.00</span>
  );
  const qtyEl = (
    <span
      className={`text-lg sm:text-xl font-bold leading-none ${qty > 0 ? 'text-th-secondary' : 'text-th-muted'}`}
    >
      {qty}
    </span>
  );
