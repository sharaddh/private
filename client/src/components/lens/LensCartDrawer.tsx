...
export default function LensCartDrawer({ onWithdrawn }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex justify-end animate-fade-in">
      <div
        className="w-full max-w-md bg-th-surface h-full flex flex-col"
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-th-hover">
          <div className="flex items-center gap-2.5">
            ...
            {count > 0 && (
              <span className="px-2 py-0.5 rounded-pill bg-primary-500 text-surface-950 text-micro font-bold">
                {count}
              </span>
            )}
          </div>
          <button
            ...
            className="p-1.5 hover:bg-th-elevated rounded-lg text-th-secondary hover:text-th-text transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        {count === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 gap-3 px-6">
            ...
            <p className="text-small text-th-muted text-center">
              Tap any lens card on the stock page to add it here.
            </p>
          </div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5">
              {items.map((item) => (
                <div
                  key={item._id}
                  className="p-3 rounded-xl border border-th-border bg-th-surface"
                >
                  <div className="flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="text-small-bold font-bold text-th-text truncate">
                        {item.coating}
                      </div>
                      <div className="text-small text-th-muted truncate">
                        {formatPower(item.lensType, item.powerKey)}
                      </div>
                      <div className="text-micro font-bold text-primary-500">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        type="button"
                        onClick={async () => {
                          if (item.quantity > 1) await updateQty(item._id, item.quantity - 1);
                          else await removeItem(item._id);
                        }}
                      ...
                      <span className="w-8 text-center text-small-bold font-bold text-th-text">
                        {item.quantity}
                      </span>
                      <button
                        type="button"
                        ...
                        disabled={
                          typeof item.available === 'number' && item.quantity >= item.available
                        }
                    ...
                    <button
                      type="button"
                      onClick={async () => {
                        if (await removeItem(item._id)) toast.info('Removed from cart');
                      }}
            ...
            <div className="border-t border-th-hover px-5 py-4 space-y-3">
              <div className="flex items-center justify-between text-small-bold">
                ...
                <span className="text-th-text font-bold">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={async () => {
                    if (await clear()) toast.info('Cart cleared');
                  }}
                  disabled={busy}
