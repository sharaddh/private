...
export default function LensStockPanel({
  ...
  onScopeChange: (s: LensStockScope) => void;
  staffMode?: boolean;
}) {
...
function LensStockPanelInner({
  ...
  const handleAddToCart = useCallback(
    async (lensType: LensType, powerKey: string) => {
      if (!selectedItem) return;
      const stockQty = getQtyFor(selectedItem, lensType, powerKey);
      const inCart =
        cart.items.find(
          (i) =>
            i.coating === selectedItem.coating && i.lensType === lensType && i.powerKey === powerKey
        )?.quantity || 0;
      if (stockQty <= 0) {
  ...
  const selectAllLowStock = useCallback(() => {
    ...
    for (const item of items) {
      const addIfLow = (lensType: LensType, key: string) => {
        ...
        if (qty < demandTarget)
          next.set(demandKey(item.coating, lensType, key), demandTarget - qty);
      };
      addIfLow('sph', '+0.00');
