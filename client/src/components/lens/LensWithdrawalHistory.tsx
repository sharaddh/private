...
export default function LensWithdrawalHistory({ open, onClose, onUpdated }: Props) {
  ...
  const handleRemoveItem = async (w: ShopLensWithdrawal, index: number) => {
    ...
    if (
      !confirm(
        `Remove "${it.coating} · ${formatPower(it.lensType, it.powerKey)}" (${it.quantity} pc) from this withdrawal? The lens${it.quantity !== 1 ? 'es' : ''} will be returned to stock.`
      )
    )
      return;
              ...
              >
                <div className="flex items-center gap-2.5 px-4 py-3 border-b border-th-hover bg-th-elevated/50">
                  ...
                  <span className="ml-auto text-small font-bold text-primary-500">
                    {w.totalQuantity} pc{w.totalQuantity !== 1 ? 's' : ''} · ₹
                    {w.totalPrice.toLocaleString('en-IN')}
                  </span>
                  <button
                    type="button"
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-pill text-micro font-bold transition-all active:scale-95 disabled:opacity-40 shrink-0 ${
                      editing
                        ? 'bg-th-elevated text-th-muted border border-th-border'
                    ...
                    <div className="space-y-2">
                      {w.items.map((it, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className={`flex items-center gap-2 rounded-lg border px-2.5 py-2 transition-all ${removed ? 'border-negative/30 bg-negative/5 opacity-60' : 'border-th-hover bg-th-elevated/30'}`}
                          >
                            <span
                              className={`flex-1 min-w-0 text-small font-bold text-th-text truncate ${removed ? 'line-through text-th-muted' : ''}`}
                            >
                            ...
                            <div className="flex flex-col items-center gap-0.5 shrink-0">
                              <div className="flex items-center gap-1">
                                <button
                                  type="button"
                                  onClick={() =>
                                    setDraft((prev) => ({ ...prev, [i]: Math.max(0, qty - 1) }))
                                  }
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    setDraft((prev) => ({ ...prev, [i]: Math.min(max, qty + 1) }))
                                  }
                              ...
                              <span className="text-micro text-th-muted">
                                {removed ? 'removed' : Number.isFinite(max) ? `max ${max}` : ''}
                              </span>
                            </div>
                          </div>
                        );
                        ...
                        {w.items.map((it, i) => (
                          ...
                          >
                            <span className="whitespace-nowrap">
                              {it.coating} · {formatPower(it.lensType, it.powerKey)} × {it.quantity}
                            </span>
                            <button
                              type="button"
