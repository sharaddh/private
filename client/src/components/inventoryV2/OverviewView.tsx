    ...
    <div className="space-y-5">
          ...
          {dashboard && (
            <>
              <div className="card px-4 py-2 flex items-center gap-2">
                ...
                <div>
                  <p className="text-[11px] text-th-secondary uppercase tracking-wide">
                    Stock Value
                  </p>
                  <p className="text-sm font-bold text-th-text">
                    {formatCurrency(dashboard.inventoryCost)}
                  </p>
                </div>
              </div>
              <div className="card px-4 py-2 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <QuickActionButton
          icon={PackagePlus}
          label="Add Stock"
          hint="Stock a purchase into an existing item"
          onClick={() => onAction({ type: 'add' })}
        />
        <QuickActionButton
          icon={Boxes}
          label="New Item"
          hint="Add a new product/SKU with stock"
          onClick={() => onAction({ type: 'new' })}
        />
        <QuickActionButton
          icon={Hand}
          label="Withdraw"
          hint="Demo, damaged or internal use"
          tone="amber"
          onClick={() => onAction({ type: 'withdraw' })}
        />
        <QuickActionButton
          icon={ClipboardList}
          label="Start Count"
          hint="Physical stock count session"
          tone="neutral"
      ...
      {alerts.length > 0 && (
        <PageSection
          ...
          actions={
            <button
              onClick={onShowStock}
              className="text-sm font-medium text-[#1ed760] hover:underline inline-flex items-center gap-1"
            >
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {alerts.map(({ v, kind }) => (
              ...
              >
                <span
                  className={`h-2 w-2 rounded-full flex-shrink-0 ${kind === 'out' ? 'bg-[#e74c3c]' : 'bg-amber-400'}`}
                />
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-th-border">
              {(dashboard?.recentActivity || []).map((m) => (
                <tr key={m._id} className="hover:bg-th-card transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span
                      className={`inline-block rounded-full px-2.5 py-1 text-[12px] font-semibold ${movementTone(m.type)}`}
                    >
                  ...
                  <td
                    className={`px-4 py-3 whitespace-nowrap text-right font-semibold ${m.quantity < 0 ? 'text-[#e74c3c]' : 'text-[#1ed760]'}`}
                  >
              ...
              {(!dashboard || dashboard.recentActivity.length === 0) && (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-sm text-th-muted">
                    No recent activity
                  </td>
                </tr>
              )}
