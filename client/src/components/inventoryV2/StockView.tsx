...
import {
  type StockActionState, VariantDetailPanel, StockEmpty,
} from "./StockActions";
...
function RowMenu({ variant, onAction, onSelect }: {
...
export default function StockView({ onAction, refreshKey, onRefresh, onNewItem }: {
  onAction: (a: StockActionState) => void;
  refreshKey: number;
  onRefresh: () => void;
  onNewItem: () => void;
  ...
  useEffect(() => { setPage(1); }, [debounced, category, stock, threshold, brandId, rackId]);
  useEffect(() => { void refetch(); }, [refreshKey, refetch]);

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="text-sm text-th-secondary">{total} item{total === 1 ? "" : "s"} in stock</p>
        <div className="flex items-center gap-2">
          <button onClick={() => void refetch()} className="btn-secondary flex items-center gap-2" aria-label="Refresh">
            <RefreshCw size={15} /> Refresh
          </button>
          <button onClick={onNewItem} className="btn-primary flex items-center gap-2" aria-label="Add item">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <select className="input-field w-auto" value={category} onChange={(e) => setCategory(e.target.value)} aria-label="Filter by category">
            <option value="All">All categories</option>
            {PRODUCT_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
          <select className="input-field w-auto" value={stock} onChange={(e) => setStock(e.target.value)} aria-label="Filter by stock">
          ...
          <select className="input-field w-auto" value={brandId} onChange={(e) => setBrandId(e.target.value)} aria-label="Filter by brand">
            <option value="">All brands</option>
            {brands.map((b) => <option key={b._id} value={b._id}>{b.name}</option>)}
          </select>
          <select className="input-field w-auto" value={rackId} onChange={(e) => setRackId(e.target.value)} aria-label="Filter by rack">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-th-hover bg-th-base">
                ...
                <th className="px-4 py-3 text-left text-[13px] font-semibold text-th-secondary uppercase tracking-wider">Item</th>
            ...
            <tbody className="divide-y divide-th-border">
              {variants.map((v) => (
                <tr key={v._id} className="hover:bg-th-card transition-colors cursor-pointer" onClick={() => setSelected(v)}>
                  <td className="px-4 py-3 whitespace-nowrap font-mono text-sm text-th-text">{v.sku}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-th-text">{itemLabel(v)}</td>
                  <td className="px-4 py-3 whitespace-nowrap"><span className="badge badge-blue">{v.category || "—"}</span></td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1">
                      <button type="button" onClick={() => onAction({ type: "add", variant: v })} className="p-1.5 hover:bg-[#1ed760]/10 rounded-lg text-[#1ed760]" aria-label="Add stock">
                        <PackagePlus size={15} />
                      </button>
