  ...
  return (
    <div className="space-y-5">
      <CountsSection
        ...
        onCountCreateHandled={onCountCreateHandled}
      />
...
function RacksSection({ refreshKey }: { refreshKey: number }) {
  ...
  const [modal, setModal] = useState<
    'create' | { _id: string; code: string; name?: string; section?: string } | null
  >(null);
      ...
      {loading && racks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {racks.map((r) => (
            <div
              key={r._id}
              className="bg-th-base rounded-[8px] border border-th-border p-4 flex flex-col gap-3"
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {r.section && (
                    <span className="inline-block mt-1 rounded-full bg-th-hover px-2 py-0.5 text-[12px] text-th-secondary">
                      Section {r.section}
                    </span>
                  )}
                </div>
                <button
                  onClick={() =>
                    setModal({ _id: r._id, code: r.code, name: r.name, section: r.section })
                  }
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-th-secondary flex items-center gap-1.5">
                  <Boxes size={14} /> {r.variants ?? 0} variants
                </span>
                <span className="text-th-text font-semibold">{r.units ?? 0} units</span>
              </div>
              <button
                onClick={() => setSelectedId(r._id)}
                className="mt-auto text-sm font-medium text-[#1ed760] hover:underline text-left flex items-center gap-1.5"
              >
                  ...
                  {items.map((it) => (
                    <tr key={it._id}>
                      ...
                      <td className="px-3 py-2 text-sm text-th-secondary">
                        {it.brandName} {it.model} {it.color}
                      </td>
                      <td className="px-3 py-2 text-right text-sm text-th-text">
                        {it.stockQuantity}
                      </td>
                    </tr>
                  ))}
            ...
            <div key={b._id} className="card p-4 group">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                <div>
                  ...
                  <p className="text-[11px] text-th-secondary">Items</p>
                </div>
                <div>
                ...
                <div>
                  <p
                    className={`text-lg font-bold ${b.lowStock > 0 ? 'text-amber-400' : 'text-[#1ed760]'}`}
                  >
                    {b.lowStock}
                  </p>
...
function CountsSection({
  ...
  return (
    ...
    >
            ...
            >
              <span className="text-sm text-th-text">
                ...
                <span className="font-semibold">
                  {s.rackLabel || rackCode(s.rackId) || s.rackId.slice(-6)}
                </span>
              </span>
              <span
                className={`inline-block rounded-full px-2.5 py-1 text-[12px] font-semibold ${STATUS_BADGE[s.status]}`}
              >
                {COUNT_STATUS_LABELS[s.status]}
              </span>
              ...
              <span className="text-sm text-th-secondary">
                Counted <span className="text-th-text font-semibold">{s.countedUnits}</span> /{' '}
                {s.expectedUnits}
              </span>
      ...
      {sessions.length > 0 && (
        <div className="mt-3">
          <Pagination page={page} pages={pages} total={total} pageSize={limit} onPage={setPage} />
        </div>
      )}
      ...
      <div className="w-full max-w-md rounded-2xl bg-th-surface border border-th-border shadow-xl p-6 space-y-4">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              <ClipboardList size={15} /> {loading ? 'Creating...' : 'Start Count'}
            </button>
          </div>
        </form>
      </div>
...
function CountDetailModal({
  ...
  async function handleSave(): Promise<void> {
    ...
    const entries = detail.entries.map((e) => ({
      ...
      countedQuantity: counts[e.variantId] ?? e.countedQuantity,
    }));
          ...
          <div>
            <h3 className="text-lg font-semibold text-th-text">
              ...
              <span className="font-mono">
                {session.rackLabel || rackCode || session.rackId.slice(-6)}
              </span>
            </h3>
            <p className="text-sm text-th-secondary mt-1">
              Status{' '}
              <span
                className={`inline-block rounded-full px-2 py-0.5 text-[12px] font-semibold ${STATUS_BADGE[session.status]}`}
              >
                {COUNT_STATUS_LABELS[session.status]}
              </span>
            ...
            <p className="text-sm text-th-secondary mt-1">
              ...
              <span className="font-semibold text-th-text">
                {isDraft ? liveCounted : session.countedUnits}
              </span>{' '}
              / {session.expectedUnits} units
              {isDraft && liveCounted !== session.expectedUnits && (
                <span
                  className={`ml-2 text-[12px] font-semibold ${liveCounted > session.expectedUnits ? 'text-[#1ed760]' : 'text-[#e74c3c]'}`}
                >
                  (
                  {liveCounted > session.expectedUnits
                    ? `+${liveCounted - session.expectedUnits}`
                    : liveCounted - session.expectedUnits}{' '}
                  vs system)
                </span>
              ...
              {detail.entries.map((e: CountEntry) => {
                ...
                return (
                  <tr key={e.variantId}>
                    ...
                    <td className="px-3 py-2 text-sm text-th-secondary max-w-[220px] truncate">
                      {itemLabel(e)}
                    </td>
                    <td className="px-3 py-2 text-right text-sm text-th-text">
                      {e.expectedQuantity}
                      ...
                      {isDraft ? (
                        <input
                          ...
                          onChange={(ev) =>
                            setCounts((prev) => ({
                              ...prev,
                              [e.variantId]: Number(ev.target.value),
                            }))
                          }
                    ...
                    <td className="px-3 py-2 text-right">
                      <span
                        className={`text-sm font-semibold ${diff > 0 ? 'text-[#1ed760]' : diff < 0 ? 'text-[#e74c3c]' : 'text-th-muted'}`}
                      >
                        {diff > 0 ? `+${diff}` : diff}
                      </span>
                    </td>
          ...
          {isDraft ? (
            <>
              <button
                ...
                disabled={cancelling}
                className="btn-ghost flex items-center gap-2 text-[#e74c3c]"
              >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-th-surface border border-th-border shadow-xl p-6 space-y-4">
            ...
            <p className="text-sm text-th-secondary">
              This will adjust stock to match the counted quantities and record movements for all
              differences. This cannot be undone.
            </p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirmComplete(false)} className="btn-ghost">
                Cancel
              </button>
              <button onClick={() => void handleComplete()} className="btn-primary">
                Complete
              </button>
            </div>
          </div>
