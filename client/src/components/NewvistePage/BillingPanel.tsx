...
export default function BillingPanel({
  ...
  function addInventoryItem(s: any) {
    const desc =
      `${s.brand || ''} ${s.model || ''}${s.color ? ` (${s.color})` : ''}`.trim() ||
      s.sku ||
      'Item';
    setBillItems((prev) => [
      ...prev,
      ...
      <div className="bg-th-surface rounded-lg shadow-lg p-5">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            placeholder={uiT(
              'Search inventory (SKU, brand, model)...',
              'इन्वेंटरी खोजें (SKU, ब्रांड, मॉडल)...'
            )}
        ...
        {billItems.length === 0 ? (
          <div className="flex items-center justify-center min-h-[120px] rounded-md bg-th-elevated">
            <p className="text-xs font-medium text-th-secondary">
              {uiT(
                'No bill items. Add items manually or sync from the order.',
                'कोई बिल आइटम नहीं। मैन्युअल रूप से आइटम जोड़ें या ऑर्डर से सिंक करें।'
              )}
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="relative w-28">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[15px] font-bold text-th-secondary">
                        ₹
                      </span>
                      <input
                        type="number"
                    ...
                    <div className="relative w-16">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const clamped =
                            item.availableQty != null ? Math.min(raw, item.availableQty) : raw;
                  ...
                  {item.availableQty != null && (
                    <div className="mt-1.5 px-1 flex items-center gap-1">
                      <span
                        className={`text-[11px] font-semibold ${overStock ? 'text-[#e53935]' : 'text-th-secondary'}`}
                      >
                        {overStock
                          ? uiT(
                              `Only ${item.availableQty} in stock. Reduce qty to continue.`,
                              `केवल ${item.availableQty} स्टॉक में। आगे बढ़ने के लिए मात्रा घटाएं।`
                            )
                          : uiT(
                              `In stock: ${item.availableQty}`,
                              `स्टॉक में: ${item.availableQty}`
        ...
        <div className="flex items-center justify-between mt-6 pt-5 border-t border-[#1f1f1f]">
          <span className="text-sm font-bold text-th-secondary uppercase tracking-wider">
            {uiT('Total Amount', 'कुल राशि')}
          </span>
          <motion.span
            key={totalAmount}
