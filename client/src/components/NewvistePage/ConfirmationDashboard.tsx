  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-4">
        <div
          className={`w-9 h-9 rounded-sm flex items-center justify-center ${theme.bg} ${theme.text}`}
        >
          {icon}
        </div>
        ...
        <div className="space-y-2">
          ...
          {visitRemarks && (
            <div className="pt-2 mt-2 border-t border-[#1f1f1f]">
              <span className="text-[15px] font-semibold text-th-secondary uppercase tracking-wider block mb-1">
                {uiT('Remarks', 'टिप्पणी')}
              </span>
            ...
            content: (
              <div className="space-y-3">
                    ...
                    <div className="border border-[#1f1f1f] rounded-md overflow-hidden">
                      ...
                      {rows.map((r) => {
                        ...
                        return (
                          <div
                            key={r}
                            className="grid grid-cols-3 text-xs border-t border-[#1f1f1f] px-3 py-1.5"
                          >
                            <span className="text-th-secondary font-medium">{labels[r]}</span>
                            <span className="text-center text-th-text font-medium">
                              {format(rd)}
                            </span>
                            <span className="text-center text-th-text font-medium">
                              {format(ld)}
                ...
                {prescription.pd && (
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-th-secondary">
                      {uiT('Pupillary Dist (PD)', 'प्यूपिलरी डिस्ट (PD)')}
                    </span>
                ...
                {prescription.problems && (
                  <div>
                    <span className="text-[15px] font-semibold text-th-secondary uppercase tracking-wider block mb-0.5">
                      {uiT('Reported Problems', 'बताई गई समस्याएँ')}
                    </span>
                ...
                {!prescription.pd && !prescription.problems && !prescription.notes && (
                  <p className="text-sm text-th-secondary italic text-center py-4">
                    {uiT(
                      'No additional details recorded.',
                      'कोई अतिरिक्त विवरण दर्ज नहीं किया गया।'
                    )}
                  </p>
                )}
              </div>
              ...
              <div className="space-y-3">
                {orderFrames.map((f, i) => (
                  <div key={`f-${i}`} className="flex justify-between items-start gap-2">
                    <div>
                      <span className="text-[14px] font-bold text-[#ff9500] uppercase tracking-wider block">
                        {uiT('Frame', 'फ्रेम')}
                      </span>
                      <p className="text-sm font-medium text-th-text">
                        {f.brand || uiT('Unknown Brand', 'अज्ञात ब्रांड')} {f.model}
                      </p>
                      {f.color && (
                        <p className="text-xs text-th-secondary">
                          {uiT('Color', 'रंग')}: {f.color}
                        </p>
                      )}
                    </div>
                ...
                {orderLenses.map((l, i) => (
                  <div
                    key={`l-${i}`}
                    className="flex justify-between items-start gap-2 pt-2 border-t border-[#1f1f1f]"
                  >
                    <div>
                      <span className="text-[14px] font-bold text-[#ff9500] uppercase tracking-wider block">
                        {uiT('Lens', 'लेंस')}
        ...
        <div className="space-y-2">
          {billItems
            ...
            .map((item, i) => (
              <div key={i} className="flex justify-between items-center text-sm">
                ...
                <span className="font-medium text-th-text tabular-nums shrink-0">
          ...
          <div className="border-t-2 border-dashed border-[#1f1f1f] mt-3 pt-3 space-y-1.5">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold text-th-text tabular-nums">
            ...
            {discountVal > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-[#e53935] tabular-nums">
            ...
            <div className="flex justify-between text-base pt-1">
              ...
              <span className="font-black text-th-text tabular-nums">
          ...
          <div className="border-t border-[#1f1f1f] mt-3 pt-3 space-y-1.5">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span
                className={`font-black tabular-nums ${isFullyPaid ? 'text-[#1ed760]' : 'text-[#ff9500]'}`}
              >
            ...
            content: (
              <div className="space-y-4">
                {deliveryDate && (
                  <div>
                    <span className="text-[15px] font-semibold text-th-secondary uppercase tracking-wider block mb-1">
                      {uiT('Expected Delivery', 'अपेक्षित डिलीवरी')}
                    </span>
                ...
                {deliveryAddress && (
                  <div>
                    <span className="text-[15px] font-semibold text-th-secondary uppercase tracking-wider block mb-1">
                      {uiT('Shipping Address', 'शिपिंग पता')}
                    </span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 bg-[#1ed760]/10 shadow-[0_0_0_1px_#1ed760] rounded-lg p-4">
        ...
        <div>
          <h2 className="text-base font-bold text-[#1ed760]">
            {uiT('Ready to Save', 'सेव करने के लिए तैयार')}
          </h2>
          <p className="text-xs font-medium text-[#1ed760]/80">
            {uiT(
              'Please verify the details below before confirming the visit.',
              'कृपया यात्रा की पुष्टि करने से पहले नीचे दिए गए विवरण की जाँच करें।'
            )}
          </p>
        </div>
      </div>
