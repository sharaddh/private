...
export default function Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      const res = await api.post<{ access: string; refresh: string; branchId?: string }>(
        '/api/auth/login',
        { username, password }
      );
      if (res.success) {
  ...
  return (
    <div
      className="min-h-screen bg-th-base flex items-center justify-center p-4"
    ...
    >
      <div className="w-full max-w-sm animate-fade-in">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-th-secondary mt-1 font-normal">
            {uiT('Admin Login', 'एडमिन लॉगिन')}
          </p>
        </div>

        <div className="bg-th-surface rounded-lg p-6 shadow-2xl">
          {error && (
            <div className="bg-[#3d1515] border border-[#b91c1c] text-[#f87171] px-4 py-3 rounded-lg text-sm mb-4">
              {error}
            </div>
          )}
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-th-secondary mb-1.5">
                {uiT('Username', 'उपयोगकर्ता नाम')}
              </label>
              <input
                type="text"
                placeholder={uiT('Enter username', 'उपयोगकर्ता नाम दर्ज करें')}
                value={username}
            ...
            <div>
              <label className="block text-sm font-medium text-th-secondary mb-1.5">
                {uiT('Password', 'पासवर्ड')}
              </label>
              <div className="relative">
                <input
                  type={showPassword ? 'text' : 'password'}
                  placeholder={uiT('Enter password', 'पासवर्ड दर्ज करें')}
                  value={password}
            ...
            >
              {isLoading ? (
                <>
                  ...
                  {uiT('Signing in...', 'साइन इन हो रहा है...')}
                </>
              ) : (
                <>
          ...
          <p className="text-center text-sm text-th-secondary mt-5">
            {uiT('Staff?', 'स्टाफ?')}{' '}
            <Link to="/staff-login" className="text-[#1ed760] hover:underline font-medium">
              {uiT('Login here', 'यहाँ लॉगिन करें')}
            </Link>
          </p>
        </div>
      </div>
