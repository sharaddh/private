...
export default function InventoryPage() {
  ...
  const [adjust, setAdjust] = useState<{ id: string; qty: number; note: string }>({
    id: '',
    qty: 0,
    note: '',
  });
  ...
  async function handleCreateVariant(existing: InventoryItem): Promise<void> {
    ...
    try {
      ...
      toast.error(
        (err as Error).message || uiT('Failed to create variant', 'वेरिएंट बनाना विफल रहा')
      );
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (skuConflict) {
      toast.error(
        uiT(
          'This SKU already exists. Use the actions below or change the SKU.',
          'यह SKU पहले से मौजूद है। नीचे दिए विकल्पों का उपयोग करें या SKU बदलें।'
        )
      );
      return;
    }
    setIsLoading(true);
    try {
      const res = editing ? await updateItem(editing._id, form) : await createItem(form);
      if (res.success) {
        toast.success(
          editing ? uiT('Item updated', 'आइटम अपडेट हो गया') : uiT('Item added', 'आइटम जोड़ा गया')
        );
        refreshAll();
        setShowForm(false);
      } else {
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    try {
      ...
      toast.success(
        `${objects.length} ${uiT('rows ready to import', 'पंक्तियां आयात के लिए तैयार')}`
      );
    } catch (err) {
  ...
  async function runImport(): Promise<void> {
    ...
    const res = await inventoryService.importItems(normalized, importNote.trim() || 'CSV import');
    setImporting(false);
    if (res.success && res.data) {
  ...
  if (loading && activeTab === 'items' && list.length === 0 && page === 1)
    return <PageSkeleton page="inventory" />;
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-500 mt-1">
            {isStaff
              ...
              : uiT(
                  'Manage specs, sunglasses, contact lenses, and more.',
                  'चश्मा, सनग्लास, कॉन्टैक्ट लेंस और अन्य का स्टॉक प्रबंधित करें।'
                )}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-th-elevated rounded-pill p-1 w-fit">
        {(['items', 'lens'] as const).map((t) => (
          <button
            ...
            aria-label={uiT(
              t === 'items' ? 'Items' : 'Lens Stock',
              t === 'items' ? 'आइटम' : 'लेंस स्टॉक'
            )}
          ...
          {!isStaff && (
            <>
              <button
                ...
                aria-label={uiT('Import CSV', 'CSV आयात करें')}
              >
                <Upload size={18} />{' '}
                <span className="hidden sm:inline">{uiT('Import', 'आयात')}</span>
              </button>
              <button
                onClick={handleExport}
                className="btn-secondary flex items-center gap-2"
                aria-label={uiT('Export CSV', 'CSV निर्यात करें')}
              >
                <Download size={18} />{' '}
                <span className="hidden sm:inline">{uiT('Export', 'निर्यात')}</span>
              </button>
              <button
                onClick={() => setShowScanner(true)}
                className="btn-secondary flex items-center gap-2"
                aria-label={uiT('Scan', 'स्कैन करें')}
              >
                <ScanLine size={18} />{' '}
                <span className="hidden sm:inline">{uiT('Scan', 'स्कैन')}</span>
              </button>
            </>
          )}
          <button
            onClick={() => navigate('/inventory/withdraw')}
            className="btn-secondary flex items-center gap-2"
            aria-label={uiT('Withdraw Stock', 'स्टॉक निकासी')}
          >
            <Package size={18} />{' '}
            <span className="hidden sm:inline">{uiT('Withdraw', 'निकासी')}</span>
          </button>
          <button
            onClick={() => navigate('/inventory/withdraw/history')}
            className="btn-secondary flex items-center gap-2"
            aria-label={uiT('Withdrawal History', 'निकासी इतिहास')}
          >
            <History size={18} />{' '}
            <span className="hidden sm:inline">{uiT('History', 'इतिहास')}</span>
          </button>
          {!isStaff && (
            <>
              <button
                onClick={() => openAdjust()}
                className="btn-secondary flex items-center gap-2"
                aria-label={uiT('Adjust Stock', 'स्टॉक समायोजित करें')}
              >
              ...
              <button
                ...
                aria-label={uiT('Add Item', 'आइटम जोड़ें')}
              >
                <Plus size={18} />{' '}
                <span className="hidden sm:inline">{uiT('Add Item', 'आइटम जोड़ें')}</span>
              </button>
            </>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="card text-center">
              <p className="text-2xl font-bold text-th-text">
                {stats?.categoryCounts?.Sunglasses ?? 0}
              </p>
              <p className="text-sm text-th-secondary">{uiT('Sunglasses', 'सनग्लास')}</p>
            </div>
            <div className="card text-center">
              <p className="text-2xl font-bold text-th-text">
                {stats?.categoryCounts?.['Contact Lens'] ?? 0}
              </p>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs text-th-muted">
                {filteredCount} {uiT('of', 'में से')} {totalCount}
              </span>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-th-hover bg-th-base">
                      ...
                      <th className="px-4 py-3.5 text-left text-[15px] font-semibold text-th-secondary uppercase tracking-wider">
                        {uiT('Brand / Model', 'ब्रांड / मॉडल')}
                      </th>
                      <th className="px-4 py-3.5 text-left text-[15px] font-semibold text-th-secondary uppercase tracking-wider">
                        {uiT('Color / Size', 'रंग / आकार')}
                      </th>
                      <th className="px-4 py-3.5 text-left text-[15px] font-semibold text-th-secondary uppercase tracking-wider">
                        {uiT('Location', 'स्थान')}
                      </th>
                        ...
                        <tr key={row._id} className="hover:bg-th-card transition-colors">
                          ...
                          <td className="px-4 py-3.5 whitespace-nowrap">
                            <span className="flex flex-col gap-0.5">
                              <span className={`badge ${categoryLabel(row.category || 'Specs')}`}>
                                {row.category || 'Specs'}
                              </span>
                              {row.inventoryType && (
                                <span className="text-[14px] text-muted-400 capitalize">
                                  {row.inventoryType}
                                </span>
                              )}
                            </span>
                          ...
                          <td className="px-4 py-3.5 whitespace-nowrap">
                            ...
                            {row.model && (
                              <span className="text-xs text-th-secondary block">{row.model}</span>
                            )}
                          </td>
                          <td className="px-4 py-3.5 whitespace-nowrap">
                            <span className="text-sm text-th-text">{row.color || '—'}</span>
                            {row.size && (
                              <span className="text-xs text-th-secondary block">{row.size}</span>
                            )}
                          </td>
                          <td className="px-4 py-3.5 whitespace-nowrap">
                            <span
                              className={`text-xs font-semibold px-2.5 py-0.5 rounded-lg ${row.location === 'warehouse' ? 'bg-purple-500/10 text-purple-400' : 'bg-blue-500/10 text-blue-400'}`}
                            >
                            ...
                            <div className="flex items-center gap-1.5">
                              {!isStaff && (
                                <>
                                  ...
                                  <span
                                    className={`font-medium min-w-[28px] text-center ${qty > (stats?.lowStockThreshold ?? 5) ? 'text-[#1ed760]' : qty > 0 ? 'text-amber-400' : 'text-[#e74c3c]'}`}
                                  >
                              ...
                              {qty <= 0 && (
                                <span className="badge badge-red">{uiT('Out', 'खत्म')}</span>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3.5 whitespace-nowrap text-sm text-th-text">
                            ...
                            <td className="px-4 py-3.5 whitespace-nowrap">
                              <div className="flex items-center gap-1">
                                <button
                                  ...
                                  title={uiT('History', 'इतिहास')}
                                  aria-label={uiT('History', 'इतिहास')}
                                >
                                  <History size={15} />
                                </button>
                                <button
                                  ...
                                  className="p-1.5 hover:bg-th-hover rounded-lg text-th-secondary"
                                  title={uiT('Adjust Stock', 'स्टॉक समायोजित करें')}
                                  aria-label={uiT('Adjust Stock', 'स्टॉक समायोजित करें')}
                                >
                                  <Package size={15} />
                                </button>
                                <button
                                  ...
                                  className="p-1.5 hover:bg-th-hover rounded-lg text-th-secondary"
                                  title={uiT('Print Label', 'लेबल प्रिंट करें')}
                                  aria-label={uiT('Print Label', 'लेबल प्रिंट करें')}
                                >
                                  <Printer size={15} />
                                </button>
                                <button
                                  ...
                                  aria-label={uiT('Edit Item', 'आइटम संपादित करें')}
                                >
                                  <Edit2 size={15} />
                                </button>
                                <button
                                  onClick={() => setDeleteTarget(row)}
                                  className="p-1.5 hover:bg-[#e74c3c]/10 rounded-lg text-[#e74c3c]"
                                  aria-label={uiT('Delete Item', 'आइटम हटाएं')}
                                >
                                  <Trash2 size={15} />
                                </button>
                              </div>
      ...
      <Modal
        ...
        title={editing ? uiT('Edit Item', 'आइटम संपादित करें') : uiT('Add Item', 'आइटम जोड़ें')}
        size="lg"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
            <div>
              ...
              <input
                ...
                aria-label={uiT('SKU', 'SKU')}
              />
              {skuCheckEnabled && (
                <div className="mt-1.5 space-y-2">
                  {skuChecking && (
                    <p className="text-xs text-th-secondary">
                      {uiT('Checking availability...', 'उपलब्धता जाँच रहे हैं...')}
                    </p>
                  )}
                  {!skuChecking && !skuConflict && (
                    <p className="text-xs text-[#1ed760]">
                      ✓ {uiT('SKU available', 'SKU उपलब्ध है')}
                    </p>
                  )}
                  {skuConflict && skuMatch && (
                    <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 space-y-2">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs">
                          <p className="font-semibold text-amber-400">
                            {uiT('SKU already exists', 'SKU पहले से मौजूद है')}
                          </p>
                          <p className="text-th-text mt-0.5">
                            {skuMatch.brand || ''} {skuMatch.model || ''}
                            {skuMatch.color ? ` / ${skuMatch.color}` : ''} — ₹
                            {(skuMatch.sellingPrice || 0).toFixed(2)}, {uiT('Stock', 'स्टॉक')}:{' '}
                            {skuMatch.quantity || 0}
                          </p>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => handleAddStockToExisting(skuMatch)}
                          className="btn-secondary px-3 py-1.5 text-xs"
                          aria-label={uiT(
                            'Add stock to existing item',
                            'मौजूदा आइटम में स्टॉक जोड़ें'
                          )}
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => handleCreateVariant(skuMatch)}
                          disabled={variantBusy}
                          className="btn-primary px-3 py-1.5 text-xs"
                          aria-label={uiT(
                            'Create color/size variant with a new SKU',
                            'नए SKU के साथ रंग/आकार वेरिएंट बनाएं'
                          )}
                        >
                          {variantBusy
                            ? uiT('Generating...', 'बना रहे हैं...')
                            : uiT('Create color/size variant', 'रंग/आकार वेरिएंट बनाएं')}
                        </button>
                        <button
                          type="button"
                          onClick={() => handleEditExisting(skuMatch)}
                          className="btn-secondary px-3 py-1.5 text-xs"
                          aria-label={uiT('Edit existing item', 'मौजूदा आइटम संपादित करें')}
                        >
              ...
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                  setForm({
                    ...
                    inventoryType: INVENTORY_TYPE_BY_CATEGORY[e.target.value] || 'other',
                  })
                }
                aria-label={uiT('Category', 'श्रेणी')}
              >
            ...
            {form.category === 'Contact Lens' && (
              <>
                ...
                <div className="md:col-span-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-th-muted mb-2">
                    {uiT('Lens details (optional)', 'लेंस विवरण (वैकल्पिक)')}
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-4 gap-y-3 rounded-lg border border-th-border p-3">
                    <div>
                      ...
                      <input
                        className="input-field"
                        value={form.lensIndex}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setForm({ ...form, lensIndex: e.target.value })
                        }
                        placeholder="e.g. 1.6"
                        aria-label={uiT('Index', 'इंडेक्स')}
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-th-secondary mb-1">
                      ...
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setForm({ ...form, lensCoating: e.target.value })
                        }
                        placeholder="e.g. Anti-glare"
                        aria-label={uiT('Coating', 'कोटिंग')}
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-th-secondary mb-1">
                        {uiT('Add Power', 'ऐड पावर')}
                      </label>
                      <input
                        className="input-field"
                        value={form.addPower}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setForm({ ...form, addPower: e.target.value })
                        }
                        placeholder="e.g. +2.00"
                        aria-label={uiT('Add Power', 'ऐड पावर')}
                      />
                    </div>
                    {(['Right', 'Left'] as const).map((side) => {
                      const isR = side === 'Right';
                      const base = isR
                        ? { sph: 'sphRight', cyl: 'cylRight', axis: 'axisRight' }
                        : { sph: 'sphLeft', cyl: 'cylLeft', axis: 'axisLeft' };
                      const sideLabel = isR
                      ...
                      return (
                        <React.Fragment key={side}>
                          <div>
                            ...
                            <input
                              className="input-field"
                              value={form[base.sph as keyof InventoryFormData] as string}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                setForm({ ...form, [base.sph]: e.target.value })
                              }
                              placeholder="e.g. -2.50"
                              aria-label={`${sideLabel} SPH`}
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-th-secondary mb-1">
                            ...
                            <input
                              className="input-field"
                              value={form[base.cyl as keyof InventoryFormData] as string}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                setForm({ ...form, [base.cyl]: e.target.value })
                              }
                              placeholder="e.g. -0.75"
                              aria-label={`${sideLabel} CYL`}
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-th-secondary mb-1">
                            ...
                            <input
                              className="input-field"
                              value={form[base.axis as keyof InventoryFormData] as string}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                setForm({ ...form, [base.axis]: e.target.value })
                              }
                              placeholder="e.g. 180"
                              aria-label={`${sideLabel} Axis`}
                            />
                          </div>
                        </React.Fragment>
                      );
            ...
            <div>
              ...
              <input
                ...
                placeholder={uiT('Supplier name', 'आपूर्तिकर्ता का नाम')}
                aria-label={uiT('Supplier', 'आपूर्तिकर्ता')}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-th-secondary mb-1.5">
            ...
            <div>
              ...
              <input
                ...
                aria-label={uiT('Quantity', 'मात्रा')}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-th-secondary mb-1.5">
                {uiT('Purchase Price (₹)', 'खरीद मूल्य (₹)')}
              </label>
              <input
                type="number"
                step="0.01"
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setForm({ ...form, purchasePrice: Math.max(0, Number(e.target.value)) })
                }
                aria-label={uiT('Purchase Price', 'खरीद मूल्य')}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-th-secondary mb-1.5">
                {uiT('Selling Price (₹)', 'बिक्री मूल्य (₹)')}
              </label>
              <input
                type="number"
                step="0.01"
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setForm({ ...form, sellingPrice: Math.max(0, Number(e.target.value)) })
                }
                aria-label={uiT('Selling Price', 'बिक्री मूल्य')}
              />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-th-secondary mb-1.5">
                {uiT('Description', 'विवरण')}
              </label>
              <textarea
                className="input-field"
                rows={2}
                ...
                aria-label={uiT('Description', 'विवरण')}
              />
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t border-th-border">
            <button
              type="submit"
              disabled={isLoading || skuConflict}
      ...
      >
        <form onSubmit={handleAdjustStock} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-th-secondary mb-1.5">
              {uiT('Select Item', 'आइटम चुनें')}
            </label>
            <select
              className="input-field"
              value={adjust.id}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setAdjust({ ...adjust, id: e.target.value })
              }
              aria-label={uiT('Select Item', 'आइटम चुनें')}
            >
              <option value="">{uiT('Choose item', 'आइटम चुनें')}</option>
              {list.map((it: InventoryItem) => (
                <option key={it._id} value={it._id}>
                  {it.sku} - {it.brand} ({it.quantity || 0} {uiT('in stock', 'स्टॉक में')})
                </option>
              ))}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-th-secondary mb-1.5">
              {uiT('Quantity Change (+/-)', 'मात्रा परिवर्तन (+/-)')}
            </label>
            <input
              type="number"
              className="input-field"
              value={adjust.qty}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                setAdjust({ ...adjust, qty: Number(e.target.value) })
              }
              placeholder="+5 or -3"
              aria-label={uiT('Quantity Change', 'मात्रा परिवर्तन')}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-th-secondary mb-1.5">
            ...
            <input
              ...
              placeholder={uiT('Reason for adjustment...', 'समायोजन का कारण...')}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-th-border">
            <button
              ...
              aria-label={
                isLoading
                  ? uiT('Saving...', 'सहेज रहे हैं...')
                  : uiT('Apply Adjustment', 'समायोजन लागू करें')
              }
            >
              {isLoading
                ? uiT('Saving...', 'सहेज रहे हैं...')
      ...
      <Modal
        ...
        title={uiT('Import Stock (CSV)', 'स्टॉक आयात करें (CSV)')}
        size="lg"
      >
        <div className="space-y-4">
          <p className="text-sm text-th-secondary">
            {uiT('Upload a CSV with columns:', 'कॉलम वाली CSV फ़ाइल अपलोड करें:')}{' '}
            <code className="text-xs bg-th-elevated px-1.5 py-0.5 rounded">
              sku, category, brand, model, color, size, gender, supplier, quantity, purchasePrice,
              sellingPrice, location, description
            </code>
          </p>
          <p className="text-xs text-th-muted">
            {uiT(
              'Only SKU is required. Existing items get their stock increased; new SKUs are added automatically.',
              'केवल SKU आवश्यक है। मौजूदा आइटम का स्टॉक बढ़ जाता है; नए SKU अपने आप जुड़ जाते हैं।'
            )}
          </p>
          ...
          <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-th-border hover:border-primary-500/50 rounded-xl p-6 cursor-pointer transition-colors">
            ...
            <input
              ...
              aria-label={uiT('Choose CSV file', 'CSV फ़ाइल चुनें')}
            />
          </label>
            ...
            <>
              <div className="flex items-center justify-between text-sm">
                <span className="text-th-text font-medium">
                  {importRows.length} {uiT('rows parsed', 'पंक्तियां पार्स हुईं')}
                </span>
              ...
              <div>
                <label className="block text-sm font-medium text-th-secondary mb-1.5">
                  {uiT('Note (optional)', 'नोट (वैकल्पिक)')}
                </label>
                <input
                  className="input-field"
                  value={importNote}
                  ...
                  placeholder={uiT('e.g. January stock delivery', 'जैसे: जनवरी स्टॉक डिलीवरी')}
                  aria-label={uiT('Note', 'नोट')}
                />
              </div>
              ...
              {importResult && (
                <div className="rounded-lg border border-th-border p-4 space-y-2">
                  <p className="text-sm font-semibold text-th-text">
                    {uiT('Import result', 'आयात परिणाम')}
                  </p>
                  <div className="flex flex-wrap gap-3 text-sm">
                    <span className="text-[#1ed760] font-medium">
                      +{importResult.created} {uiT('created', 'नए')}
                    </span>
                    <span className="text-amber-400 font-medium">
                      ↻ {importResult.updated} {uiT('updated', 'अपडेट')}
                    </span>
                    <span className="text-[#e74c3c] font-medium">
                      ✕ {importResult.skipped} {uiT('skipped', 'छोड़े गए')}
                    </span>
                  </div>
                  {importResult.errors.length > 0 && (
                    <div className="max-h-32 overflow-y-auto rounded-lg bg-th-elevated p-2">
                      {importResult.errors.map((e, idx) => (
                        <p key={idx} className="text-xs text-[#e74c3c] py-0.5">
                          Row {e.row}: {e.message}
                        </p>
                      ))}
                    </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                >
                  {importing
                    ? uiT('Importing...', 'आयात हो रहा है...')
                    : uiT('Import Stock', 'स्टॉक आयात करें')}
                </button>
              </div>
            </>
          )}
      ...
      <Modal
        ...
        title={`${uiT('Stock History', 'स्टॉक इतिहास')} — ${historyItem?.sku ?? ''}`}
      >
        <div className="space-y-2">
          {historyItem && (!historyItem.stockHistory || historyItem.stockHistory.length === 0) && (
            <p className="text-sm text-th-muted text-center py-8">
              {uiT('No stock history yet', 'अभी कोई स्टॉक इतिहास नहीं')}
            </p>
          )}
          {historyItem?.stockHistory?.map((h: StockHistoryEntry, idx: number) => (
            <div
              key={idx}
              className="flex items-start justify-between gap-3 rounded-lg border border-th-border p-3"
            >
              <div className="min-w-0">
                <p className="text-sm font-medium text-th-text">
                  ...
                  {h.qty}
                  <span
                    className={`ml-2 text-xs font-semibold px-2 py-0.5 rounded-pill ${h.qty > 0 ? 'bg-[#1ed760]/10 text-[#1ed760]' : 'bg-[#e74c3c]/10 text-[#e74c3c]'}`}
                  >
      ...
      <Modal
        ...
        title={uiT('Scan Barcode / QR', 'बारकोड / QR स्कैन करें')}
        size="sm"
      >
      ...
      <ConfirmDialog
        ...
        message={
          uiT('Delete', 'हटाएं') +
          ` ${deleteTarget?.sku ?? ''}? ` +
          uiT('This cannot be undone.', 'यह वापस नहीं किया जा सकता।')
        }
