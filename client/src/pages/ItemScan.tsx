...
export default function ItemScan() {
  ...
  useEffect(() => {
    if (!code) return;
    api
      .get<InventoryItem>('/api/inventory/qr/' + code)
      .then((res) => {
        ...
        <div className="flex items-start gap-5">
          <div className="w-24 h-24 bg-th-elevated rounded-lg flex items-center justify-center flex-shrink-0 p-2">
            <canvas
              ...
              aria-label={uiT('QR code for item', 'आइटम के लिए QR कोड')}
            />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-th-text">
                  {item.brand || uiT('Item', 'आइटम')} {item.model || ''}
                </h1>
              ...
              <div className="flex gap-2">
                <button
                  onClick={handlePrint}
                  aria-label={uiT('Print item label', 'आइटम लेबल प्रिंट करें')}
                  className="bg-transparent border border-[#727272] text-th-text hover:border-th-text text-xs font-bold uppercase tracking-wider rounded-lg px-4 py-1.5 flex items-center gap-1.5 transition-all"
                >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-5">
              ...
              <div className="bg-th-elevated rounded-sm p-3">
                ...
                <p
                  className={`body font-semibold ${(item.quantity || 0) > 5 ? 'text-[#1ed760]' : (item.quantity || 0) > 0 ? 'text-amber-400' : 'text-[#e74c3c]'}`}
                >
                  {item.quantity || 0}
                </p>
              </div>
