...
function HlsPlayer({
  ...
  return (
    <video ref={videoRef} className={className} style={style} controls muted autoPlay playsInline />
  );
}
...
function CameraCard({
  ...
  const copySn = useCallback(() => {
    navigator.clipboard.writeText(cam.serialNumber);
  }, [cam.serialNumber]);
  ...
  return (
    <div
      ref={containerRef}
      className={`bg-th-surface rounded-xl shadow-lg overflow-hidden transition-all duration-300 flex flex-col ${expanded ? 'fixed inset-4 z-40' : ''}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-th-hover">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span
            className="text-[14px] px-1.5 py-0.5 rounded font-medium flex-shrink-0"
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button
            onClick={copySn}
            title="Copy SN"
            className="p-1.5 rounded-lg text-th-secondary hover:text-th-text hover:bg-th-hover transition-colors"
          >
            <Copy size={14} />
          </button>
          <button
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-th-secondary hover:text-red-400 hover:bg-red-400/10 transition-colors"
          >
            <Trash2 size={14} />
          </button>
          <button
            onClick={toggleExpand}
            title={expanded ? t('Minimize', 'छोटा करें') : t('Maximize', 'बड़ा करें')}
            className="p-1.5 rounded-lg text-th-secondary hover:text-th-text hover:bg-th-hover transition-colors"
          >
          ...
          <div className="absolute inset-0 flex items-center justify-center z-10">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-th-secondary">
                {cam.status === 'connecting'
                  ? t('Connecting via P2P...', 'P2P से कनेक्ट हो रहा है...')
                  : t('Loading feed...', 'फ़ीड लोड हो रहा है...')}
              </span>
            </div>
          </div>
        )}
        ...
        {cam.status === 'error' && (
          <div className="absolute inset-0 flex items-center justify-center p-4 z-10">
            <div className="flex flex-col items-center gap-3 text-center max-w-xs">
              ...
              <div>
                <p className="text-sm font-semibold text-th-text mb-1">
                  {t('Connection failed', 'कनेक्शन विफल')}
                </p>
                <p className="text-xs text-th-secondary leading-relaxed">
                  {cam.lastError || 'Relay server error'}
                </p>
              </div>
            </div>
          </div>
        ...
        {loadError && cam.status !== 'error' && (
          <div className="absolute inset-0 flex items-center justify-center p-4 z-10">
            <div className="flex flex-col items-center gap-3 text-center max-w-xs">
              ...
              <div>
                <p className="text-sm font-semibold text-th-text mb-1">
                  {t('Cannot load feed', 'फ़ीड लोड नहीं हो सका')}
                </p>
              </div>
              <button
                onClick={() => {
        ...
        {cam.status === 'offline' && (
          <div className="absolute inset-0 flex items-center justify-center p-4 z-10">
            <div className="flex flex-col items-center gap-3 text-center max-w-xs">
              ...
              <div>
                <p className="text-sm font-semibold text-th-text mb-1">
                  {t('Camera offline', 'कैमरा ऑफ़लाइन')}
                </p>
                <p className="text-xs text-th-secondary leading-relaxed">
                  {t(
                    'P2P relay is not connected. Check if the relay server is running.',
                    'P2P रिले कनेक्ट नहीं है। जाँचें कि रिले सर्वर चल रहा है।'
                  )}
                </p>
              </div>
            </div>
...
function AddEditModal({
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={editCam ? t('Edit Camera', 'कैमरा संपादित करें') : t('Add Camera', 'कैमरा जोड़ें')}
      size="md"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder={t(
              'Find on camera label or Easyviewer app',
              'कैमरे के लेबल या Easyviewer ऐप पर देखें'
            )}
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button
            type="submit"
            disabled={!name.trim() || !serialNumber.trim()}
...
export default function Cameras() {
  ...
  const handleSave = useCallback(
    async (data: { name: string; serialNumber: string; username: string; password: string }) => {
      if (editCam) {
        ...
        if (res.success) {
          toast.success(
            t(
              'Camera added — connecting via P2P...',
              'कैमरा जोड़ा गया — P2P से कनेक्ट हो रहा है...'
            )
          );
          fetchCameras();
        } else {
  ...
  const handleEdit = useCallback((cam: CameraDoc) => {
    setEditCam(cam);
    setModalOpen(true);
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={t('Delete Camera', 'कैमरा हटाएँ')}
        size="sm"
      >
