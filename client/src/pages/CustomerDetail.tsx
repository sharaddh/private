...
const inputCls =
  'w-full bg-th-hover text-th-text placeholder-th-secondary px-3 py-2.5 rounded-md text-base outline-none focus:ring-1 focus:ring-[#1ed760] transition-shadow';
...
function EyeRow({ label, data }: { label: string; data: any }) {
  const sph =
    data.sph != null
      ? data.sph > 0
        ? `+${Number(data.sph).toFixed(2)}`
        : Number(data.sph).toFixed(2)
      : '—';
  const cyl =
    data.cyl != null
      ? data.cyl > 0
        ? `+${Number(data.cyl).toFixed(2)}`
        : Number(data.cyl).toFixed(2)
      : '—';
  const axis = data.axis != null ? `× ${data.axis}` : '—';
  return (
    <div className="text-base text-th-secondary mb-1.5 last:mb-0">
      <span className="font-bold text-th-text bg-th-border px-1.5 py-0.5 rounded text-sm mr-1.5">
        {label}
      </span>
      <span className="text-th-secondary">SPH</span>{' '}
      <span className="font-semibold text-th-text">{sph}</span>
      <span className="text-[#535353] mx-1">|</span>
      <span className="text-th-secondary">CYL</span>{' '}
      <span className="font-semibold text-th-text">{cyl}</span>
      <span className="text-[#535353] mx-1">|</span>
      <span className="text-th-secondary">AXIS</span>{' '}
      <span className="font-semibold text-th-text">{axis}</span>
      {data.va && (
        <>
          <span className="text-[#535353] mx-1">|</span>
          <span className="text-th-secondary">VA</span>{' '}
          <span className="font-semibold text-th-text">{data.va}</span>
        </>
      )}
    </div>
  );
...
export default function CustomerDetail() {
  ...
  const [tab, setTab] = useState<'overview' | 'visits' | 'prescriptions' | 'bills' | 'orders'>(
    'overview'
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get<any[]>(`/api/bills?customerId=${id}`),
      ...
      if (b.success)
        setBills((b.data as any)?.data || ((Array.isArray(b.data) ? b.data : []) as any[]));
      if (o.success)
        setOrders((o.data as any)?.data || ((Array.isArray(o.data) ? o.data : []) as any[]));
        ...
        tags:
          typeof editForm.tags === 'string'
            ? editForm.tags
                ...
                .map((t: string) => t.trim())
                .filter(Boolean)
            : editForm.tags || undefined,
      };
      const res = await api.put<any>(`/api/customers/${id}`, payload);
      if (res.success) {
        setCustomer(res.data);
        setEditing(false);
        toast.success(uiT('Profile updated', 'प्रोफ़ाइल अपडेट हो गई'));
      } else {
  ...
  async function handleBillAdvanceSave() {
    ...
    try {
      ...
      if (res.success) {
        ...
        const paymentsList =
          (paymentsRes.data as any)?.data ||
          ((Array.isArray(paymentsRes.data) ? paymentsRes.data : []) as any[]);
        if (paymentsRes.success && paymentsList.length > 0) {
          await api.put(`/api/payments/${paymentsList[0]._id}`, {
  ...
  function openVisitDetail(v: any) {
    setSelectedVisit(v);
    setEditVisitForm({
      visitDate: v.visitDate ? toDateKey(v.visitDate) : '',
  ...
  function openRxEdit() {
    ...
    setEditRxForm({
      rightEye: {
        dv: {
          sph: p?.rightEye?.dv?.sph ?? '',
          cyl: p?.rightEye?.dv?.cyl ?? '',
          axis: p?.rightEye?.dv?.axis ?? '',
          va: p?.rightEye?.dv?.va || '',
        },
        nv: {
          sph: p?.rightEye?.nv?.sph ?? '',
          cyl: p?.rightEye?.nv?.cyl ?? '',
          axis: p?.rightEye?.nv?.axis ?? '',
          va: p?.rightEye?.nv?.va || '',
        },
        pc: {
          sph: p?.rightEye?.pc?.sph ?? '',
          cyl: p?.rightEye?.pc?.cyl ?? '',
          axis: p?.rightEye?.pc?.axis ?? '',
          va: p?.rightEye?.pc?.va || '',
        },
      },
      leftEye: {
        dv: {
          sph: p?.leftEye?.dv?.sph ?? '',
          cyl: p?.leftEye?.dv?.cyl ?? '',
          axis: p?.leftEye?.dv?.axis ?? '',
          va: p?.leftEye?.dv?.va || '',
        },
        nv: {
          sph: p?.leftEye?.nv?.sph ?? '',
          cyl: p?.leftEye?.nv?.cyl ?? '',
          axis: p?.leftEye?.nv?.axis ?? '',
          va: p?.leftEye?.nv?.va || '',
        },
        pc: {
          sph: p?.leftEye?.pc?.sph ?? '',
          cyl: p?.leftEye?.pc?.cyl ?? '',
          axis: p?.leftEye?.pc?.axis ?? '',
          va: p?.leftEye?.pc?.va || '',
        },
      },
      pd: p?.pd || '',
  ...
  async function sendPrescriptionWhatsApp(rx?: any) {
    ...
    if (!customer?.mobile) {
      toast.error(uiT('No mobile number', 'कोई मोबाइल नंबर नहीं'));
      return;
    }
    if (!rxData) {
      toast.error(uiT('No prescription data', 'कोई प्रिस्क्रिप्शन डेटा नहीं'));
      return;
    }
    const fullNum = normalizeWhatsAppPhone(customer.mobile || '');
    if (!fullNum) {
      toast.error(uiT('Invalid mobile number', 'अमान्य मोबाइल नंबर'));
      return;
    }
    ...
    try {
      ...
      const res = await whatsappService.sendMedia({
        phone: fullNum,
        base64,
        filename: `Prescription-${customer.customerId || 'rx'}.pdf`,
        caption,
        mimetype: 'application/pdf',
      });
      if (res.success && res.data?.sent) {
        toast.success(uiT('Prescription sent', 'प्रिस्क्रिप्शन भेजा गया'));
      } else if (res.data?.queued) {
        toast.info(
          uiT(
            'WhatsApp not ready — will send when connected',
            'WhatsApp तैयार नहीं — कनेक्ट होने पर भेजा जाएगा'
          )
        );
      } else {
        toast.error(res.message || uiT('Failed to send', 'भेजने में विफल'));
      }
    } catch (e: any) {
      toast.error(e.message || uiT('Failed to send', 'भेजने में विफल'));
    } finally {
      setSendingRx(false);
    }
  }
  ...
  function cleanEyeEdit(eye: any) {
    ...
    for (const sub of ['dv', 'nv', 'pc'] as const) {
      const s = eye[sub];
      if (!s) continue;
      const has = ['sph', 'cyl', 'axis', 'va'].some(
        (k) => s[k] !== '' && s[k] != null && s[k] !== ''
      );
      if (has) {
        out[sub] = {};
  ...
  async function handleRxSave() {
    ...
    try {
      const payload: any = {
        rightEye: {
          dv: cleanEyeEdit(editRxForm.rightEye?.dv),
          nv: cleanEyeEdit(editRxForm.rightEye?.nv),
          pc: cleanEyeEdit(editRxForm.rightEye?.pc),
        },
        leftEye: {
          dv: cleanEyeEdit(editRxForm.leftEye?.dv),
          nv: cleanEyeEdit(editRxForm.leftEye?.nv),
          pc: cleanEyeEdit(editRxForm.leftEye?.pc),
        },
      ...
      if (res.success) {
        setPrescriptions((prev: any[]) =>
          prev.map((p: any) => (p._id === res.data._id ? res.data : p))
        );
        setEditingRx(false);
  ...
  function openOrderEdit() {
    ...
    setEditOrderForm({
      ...
      frameModel: o?.frameModel || '',
      frameColor: o?.frameColor || '',
      frameSize: o?.frameSize || '',
      framePrice: o?.framePrice ?? '',
      lens: o?.lens || '',
      lensBrand: o?.lensBrand || '',
      lensType: o?.lensType || '',
      lensIndex: o?.lensIndex || '',
      lensPrice: o?.lensPrice ?? '',
      coating: o?.coating || '',
      coatingPrice: o?.coatingPrice ?? '',
      accessories: o?.accessories?.join(', ') || '',
      quantity: o?.quantity ?? 1,
      deliveryDate: o?.deliveryDate ? toDateKey(o.deliveryDate) : '',
      status: o?.status || 'Draft',
      labAssigned: o?.labAssigned || '',
      labExpectedDate: o?.labExpectedDate ? toDateKey(o.labExpectedDate) : '',
      labRemarks: o?.labRemarks || '',
    });
  ...
  async function handleOrderSave() {
    ...
    try {
      const payload: any = {
        frame: editOrderForm.frame || undefined,
        frameBrand: editOrderForm.frameBrand || undefined,
        frameModel: editOrderForm.frameModel || undefined,
        frameColor: editOrderForm.frameColor || undefined,
        frameSize: editOrderForm.frameSize || undefined,
        framePrice: editOrderForm.framePrice !== '' ? Number(editOrderForm.framePrice) : undefined,
        lens: editOrderForm.lens || undefined,
        lensBrand: editOrderForm.lensBrand || undefined,
        lensType: editOrderForm.lensType || undefined,
        lensIndex: editOrderForm.lensIndex || undefined,
        lensPrice: editOrderForm.lensPrice !== '' ? Number(editOrderForm.lensPrice) : undefined,
        coating: editOrderForm.coating || undefined,
        coatingPrice:
          editOrderForm.coatingPrice !== '' ? Number(editOrderForm.coatingPrice) : undefined,
        accessories: editOrderForm.accessories
          ? editOrderForm.accessories
              .split(',')
              .map((s: string) => s.trim())
              .filter(Boolean)
          : undefined,
        quantity: Number(editOrderForm.quantity) || 1,
        deliveryDate: editOrderForm.deliveryDate || undefined,
        status: editOrderForm.status || undefined,
        labAssigned: editOrderForm.labAssigned || undefined,
        labExpectedDate: editOrderForm.labExpectedDate || undefined,
  ...
  function addBillItem() {
    setEditBillForm((f: any) => ({
      ...f,
      items: [...(f.items || []), { description: '', quantity: '', unitPrice: '' }],
    }));
  }
  ...
  async function handleBillSave() {
    ...
    try {
      ...
      if (editBillForm.discount !== '' && editBillForm.discount != null)
        payload.discount = Number(editBillForm.discount);
      if (editBillForm.tax !== '' && editBillForm.tax != null)
        payload.tax = Number(editBillForm.tax);
      if (editBillForm.advancePaid !== '' && editBillForm.advancePaid != null)
        payload.advancePaid = Number(editBillForm.advancePaid);
    ...
    try {
      if (bill) {
        ...
        if (mediaRes.success && mediaRes.data?.sent) {
          toast.success(uiT('Bill sent on WhatsApp', 'बिल व्हाट्सऐप पर भेजा गया'));
  ...
  const tabs = [
    ...
    {
      key: 'prescriptions',
      label: `${uiT('Rx', 'प्रिस्क्रिप्शन')} (${prescriptions.length})`,
      icon: Eye,
    },
    ...
    <div className="min-h-screen bg-th-base text-th-text px-8 py-6 max-w-[1400px] mx-auto">
      ...
      <div className="bg-th-surface rounded-xl mb-5 p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2.5">
                {customer.mobile && (
                  <span className="flex items-center gap-1 text-base text-th-secondary">
                ...
                {customer.city && (
                  <span className="flex items-center gap-1 text-base text-th-secondary">
                    <MapPinned size={12} /> {customer.city}
                  </span>
                )}
                {customer.age && (
              ...
              {customer.address && (
                <p className="text-base text-th-secondary mt-1.5 flex items-center gap-1">
                  <MapPin size={11} />
                  {customer.address}
                </p>
              )}
              {customer.alternateMobile && (
                <p className="text-base text-th-secondary mt-0.5 flex items-center gap-1">
                  <Phone size={11} />
                  {uiT('Alt', 'वैकल्पिक')}: {customer.alternateMobile}
                </p>
              )}
              {customer.tags?.length > 0 && (
                <div className="flex gap-1.5 mt-2.5">
                  {customer.tags.map((tag: string) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 bg-[#1ed760]/10 text-[#1ed760] text-sm font-bold uppercase tracking-wider rounded-md"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <button
            onClick={openEdit}
            className="flex items-center gap-1.5 text-base font-bold uppercase tracking-wider px-5 py-2 rounded-lg bg-th-elevated text-th-text hover:bg-th-hover active:scale-95 transition-all duration-150 shrink-0"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <div className="bg-th-surface rounded-lg flex items-center gap-3 p-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-base text-th-secondary font-bold uppercase tracking-wider">
              {uiT('Visits', 'विज़िट')}
            </p>
          </div>
        </div>
        ...
        <div className="bg-th-surface rounded-lg flex items-center gap-3 p-4">
          ...
          <div className="min-w-0">
            <p className="text-xl font-bold text-[#e8a427]">
        ...
        <div
          onClick={() => navigate(`/customers/${id}/create-visit`)}
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-base text-black/50">
              {uiT('with prescription', 'प्रिस्क्रिप्शन के साथ')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div>
              ...
              <div className="bg-th-surface rounded-lg p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-base font-bold uppercase tracking-wider text-th-text">
                    {uiT('Latest Prescription', 'नवीनतम प्रिस्क्रिप्शन')}
                  </p>
                  {latestRx && (
                    <span className="text-base text-th-secondary">
                      {new Date(latestRx.createdAt).toLocaleDateString('en-IN', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                      })}
                ...
                {latestRx ? (
                  <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="bg-th-elevated rounded-md p-3">
                        ...
                        {!latestRx.rightEye?.dv &&
                          !latestRx.rightEye?.nv &&
                          !latestRx.rightEye?.pc && (
                            <p className="text-base text-th-secondary italic">
                              {uiT('No data', 'कोई डेटा नहीं')}
                            </p>
                          )}
                      </div>
                      <div className="bg-th-elevated rounded-md p-3">
                        <p className="text-base font-bold uppercase tracking-wider text-[#e8a427] mb-1.5 flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#e8a427]" />{' '}
                          {uiT('Left Eye', 'बाईं आंख')}
                        </p>
                    ...
                    {(latestRx.pd || latestRx.notes) && (
                      <div className="flex gap-4 mt-2.5 pt-2.5 border-t border-th-border text-base text-th-secondary">
                        {latestRx.pd && (
                          <span>
                            <span className="font-medium">PD:</span> {latestRx.pd}
                          </span>
                        )}
                        {latestRx.notes && (
                          <span className="truncate">
                            <span className="font-medium">{uiT('Notes:', 'नोट्स:')}</span>{' '}
                            {latestRx.notes}
                          </span>
                        )}
                      </div>
                  ...
                  <p className="text-base text-th-secondary py-3 text-center">
                    {uiT('No prescriptions yet', 'अभी तक कोई प्रिस्क्रिप्शन नहीं')}
                  </p>
                )}
              </div>
              ...
              <div className="bg-th-surface rounded-lg p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-base font-bold uppercase tracking-wider text-th-text">
                      {uiT('Recent Visits', 'हालिया विज़िट')}
                    </p>
                  </div>
                  <button
                    onClick={() => setTab('visits')}
                    className="text-base text-[#1ed760] font-bold uppercase tracking-wider hover:underline"
                  >
                ...
                {visits.length > 0 ? (
                  <div className="space-y-2">
                    {visits.slice(0, 4).map((v: any) => {
                      ...
                      const ord = orders.find(
                        (o: any) => getVisitId(o.visitId) === getVisitId(v._id)
                      );
                      const bill = bills.find(
                        (b: any) => getVisitId(b.visitId) === getVisitId(v._id)
                      );
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {v.visitType && (
                                <span className="px-1 py-0.5 bg-th-hover text-th-secondary text-base font-bold uppercase rounded">
                                  {v.visitType}
                                </span>
                              )}
                              {v.doctorName && (
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            {rx ? (
                              <span className="text-base font-medium truncate max-w-[160px]">
                                ...
                                <span
                                  className={
                                    rxBrief && rxBrief !== 'Plain'
                                      ? 'text-th-text'
                                      : 'italic text-th-secondary'
                                  }
                                >
                                  {rxBrief || 'Plain'}
                                </span>
                            ...
                            {ord && (
                              <span
                                className={`text-base font-bold uppercase px-1 py-0.5 rounded ${ord.status === 'Delivered' ? 'bg-[#1ed760]/10 text-[#1ed760]' : ord.status === 'Cancelled' ? 'bg-red-500/10 text-red-400' : 'bg-[#e8a427]/10 text-[#e8a427]'}`}
                              >
                                {ord.status}
                              </span>
                            )}
                  ...
                  <p className="text-base text-th-secondary py-3 text-center">
                    {uiT('No visits yet', 'अभी तक कोई विज़िट नहीं')}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-th-surface rounded-lg p-5">
                ...
                {bills.length > 0 ? (
                  ...
                  <p className="text-base text-th-secondary py-3 text-center">
                    {uiT('No bills yet', 'अभी तक कोई बिल नहीं')}
                  </p>
                )}
              </div>
              ...
              <div className="bg-th-surface rounded-lg p-5">
                ...
                {orders.length > 0 ? (
                  ...
                  <p className="text-base text-th-secondary py-3 text-center">
                    {uiT('No orders yet', 'अभी तक कोई ऑर्डर नहीं')}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="bg-th-surface rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-th-text flex items-center gap-2">
                <ClipboardList size={14} className="text-[#1ed760]" />{' '}
                {uiT('All Visits', 'सभी विज़िट')} ({visits.length})
              </h3>
              <button
                onClick={() => navigate(`/customers/${id}/create-visit`)}
                className="flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-2 rounded-lg bg-[#1ed760] text-black hover:bg-[#1ed760]/90 active:scale-95 transition-transform"
              >
            ...
            {visits.length === 0 ? (
              <div className="py-10 text-center">
                ...
                <button
                  onClick={() => navigate(`/customers/${id}/create-visit`)}
                  className="mt-3 flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-2 rounded-lg bg-[#1ed760] text-black mx-auto"
                >
              ...
              <div className="space-y-1.5">
                {visits.map((v: any) => {
                  ...
                  const visitOrder = orders.find(
                    (o: any) => getVisitId(o.visitId) === getVisitId(v._id)
                  );
                  const visitBill = bills.find(
                    (b: any) => getVisitId(b.visitId) === getVisitId(v._id)
                  );
                  const rxBrief = linkedRx
                    ? compactRx(linkedRx.rightEye?.dv, linkedRx.rightEye?.nv)
                    : null;
                  const lxBrief = linkedRx
                    ? compactRx(linkedRx.leftEye?.dv, linkedRx.leftEye?.nv)
                    : null;
                  return (
                    <ShineCard
                      key={v._id}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {v.visitType && (
                              <span className="px-1.5 py-0.5 bg-th-hover text-th-secondary text-base font-bold uppercase tracking-wider rounded">
                                {v.visitType}
                              </span>
                            )}
                            {visitOrder && (
                              <span
                                className={`px-1.5 py-0.5 text-sm font-bold uppercase tracking-wider rounded ${
                                  visitOrder.status === 'Delivered'
                                    ? 'bg-[#1ed760]/10 text-[#1ed760]'
                                    : visitOrder.status === 'Ready'
                                      ? 'bg-[#6ea8fe]/10 text-[#6ea8fe]'
                                      : visitOrder.status === 'Cancelled'
                                        ? 'bg-red-500/10 text-red-400'
                                        : 'bg-[#e8a427]/10 text-[#e8a427]'
                                }`}
                              >
                                {visitOrder.status || 'Draft'}
                              </span>
                            )}
                            {visitBill && (
                              <span
                                className={`px-1.5 py-0.5 text-base font-bold tracking-wider rounded ${
                                  (visitBill.pendingAmount || 0) > 0
                                    ? 'bg-[#e8a427]/10 text-[#e8a427]'
                                    : 'bg-[#1ed760]/10 text-[#1ed760]'
                                }`}
                              >
                                ...
                                {(visitBill.pendingAmount || 0) > 0 && (
                                  <span> · ₹{visitBill.pendingAmount} due</span>
                                )}
                              </span>
                            )}
                          </div>
                          <div className="flex items-center gap-3 text-base text-th-secondary mt-0.5 flex-wrap">
                            {v.doctorName && (
                              <span className="flex items-center gap-0.5">
                                <Stethoscope size={10} /> Dr. {v.doctorName}
                              </span>
                            )}
                            {linkedRx ? (
                              <span className="flex items-center gap-1.5">
                                ...
                                <span>
                                  R:{' '}
                                  <span
                                    className={`font-medium ${rxBrief && rxBrief !== 'Plain' ? 'text-th-text' : 'italic text-th-secondary'}`}
                                  >
                                    {rxBrief || 'Plain'}
                                  </span>
                                </span>
                                ...
                                <span>
                                  L:{' '}
                                  <span
                                    className={`font-medium ${lxBrief && lxBrief !== 'Plain' ? 'text-th-text' : 'italic text-th-secondary'}`}
                                  >
                                    {lxBrief || 'Plain'}
                                  </span>
                                </span>
                              ...
                              <span className="flex items-center gap-1 italic">
                                <Eye size={10} className="text-th-secondary/50" /> No Rx
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <ChevronRight
                        size={14}
                        className="text-[#535353] group-hover:text-th-secondary transition-colors shrink-0"
                      />
                    </ShineCard>
                  );
                })}
        ...
        {tab === 'prescriptions' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-th-text flex items-center gap-2">
                <Eye size={14} className="text-purple-400" />{' '}
                {uiT('Prescriptions', 'प्रिस्क्रिप्शन')} ({prescriptions.length})
              </h3>
              <button
                onClick={() => navigate(`/customers/${id}/create-visit`)}
                className="flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-2 rounded-lg bg-[#1ed760] text-black hover:bg-[#1ed760]/90 active:scale-95 transition-transform"
              ...
              prescriptions.map((p: any) => (
                <div key={p._id} className="bg-th-surface rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <button
                      onClick={() => sendPrescriptionWhatsApp(p)}
                      disabled={sendingRx}
                      className="ml-auto flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-1.5 rounded-lg bg-[#1ed760]/10 text-[#1ed760] hover:bg-[#1ed760]/20 transition-colors disabled:opacity-50"
                    >
                      {sendingRx ? (
                        <div className="animate-spin w-3 h-3 border-2 border-[#1ed760] border-t-transparent rounded-full" />
                      ) : (
                        <MessageCircle size={11} />
                      )}{' '}
                      {uiT('Send Rx', 'प्रिस्क्रिप्शन भेजें')}
                    </button>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="bg-th-elevated rounded-md p-3">
                  ...
                  {(p.pd || p.notes) && (
                    <div className="flex gap-4 mt-3 pt-3 border-t border-th-border">
                      {p.pd && (
                        <p className="text-base text-th-secondary">
                          <span className="font-medium">PD:</span> {p.pd}
                        </p>
                      )}
                      {p.notes && (
                        <p className="text-base text-th-secondary">
                          <span className="font-medium">{uiT('Notes:', 'नोट्स:')}</span> {p.notes}
                        </p>
                      )}
                    </div>
                  )}
        ...
        {tab === 'bills' && (
          <div className="bg-th-surface rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-th-text flex items-center gap-2">
                <Receipt size={14} className="text-[#1ed760]" /> {uiT('Bills', 'बिल')} (
                {bills.length})
              </h3>
              {bills.length > 0 && (
                <div className="flex items-center gap-3 text-base">
                  ...
                  {bills.reduce((s: number, b: any) => s + (b.pendingAmount || 0), 0) > 0 && (
                    <span className="font-bold text-[#e8a427]">
                      ₹
                      {bills
                        .reduce((s: number, b: any) => s + (b.pendingAmount || 0), 0)
                        .toLocaleString()}{' '}
                      {uiT('due', 'बकाया')}
                    </span>
                  )}
              ...
              <div className="space-y-2">
                {bills.map((b: any) => {
                  ...
                  return (
                    <ShineCard
                      key={b._id}
                      className="bg-th-elevated rounded-lg p-4 hover:bg-th-hover active:scale-[0.99] cursor-pointer"
                      onClick={() => {
                        if (linkedVisit) {
                          setTab('visits');
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {b.status === 'Cancelled' && (
                              <span className="px-1.5 py-0.5 text-base font-bold uppercase tracking-wider rounded bg-red-500/10 text-red-400">
                                {uiT('Cancelled', 'रद्द')}
                              </span>
                            )}
                          </div>
                          <p className="text-base text-th-secondary mt-0.5">
                            ...
                            {itemCount > 0 && (
                              <span>
                                {' '}
                                · {itemCount}{' '}
                                {itemCount === 1 ? uiT('item', 'आइटम') : uiT('items', 'आइटम')}
                              </span>
                            )}
                            {linkedVisit && (
                              <span>
                                {' '}
                                · {uiT('Visit', 'विज़िट')}{' '}
                                {new Date(linkedVisit.visitDate).toLocaleDateString('en-IN', {
                                  day: 'numeric',
                                  month: 'short',
                                })}
                              </span>
                          ...
                          {itemSummary && (
                            <p className="text-base text-th-secondary mt-1 truncate max-w-[400px]">
                              {itemSummary}
                          ...
                          <div className="flex items-center gap-3 mt-2 text-base">
                            {hasDiscount && (
                              <span className="text-red-400">
                                {uiT('Discount', 'छूट')}: -₹{b.discount}
                              </span>
                            )}
                            {hasTax && (
                              <span className="text-[#1ed760]">
                                {uiT('GST', 'जीएसटी')}: +₹{b.tax}
                              </span>
                            )}
                            {b.advancePaid > 0 && (
                              <span className="text-[#1ed760]">
                                {uiT('Paid', 'भुगतान')}: ₹{b.advancePaid.toLocaleString()}
                              </span>
                            )}
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          ...
                          {(b.pendingAmount || 0) > 0 && (
                            <p className="text-base font-bold text-[#e8a427] mt-0.5">
                              ₹{b.pendingAmount.toLocaleString()} {uiT('due', 'बकाया')}
                            </p>
                          )}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              sendWhatsApp(customer.mobile, b);
                            }}
        ...
        {tab === 'orders' && (
          <div className="bg-th-surface rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-th-text flex items-center gap-2">
                <ShoppingCart size={14} className="text-[#e8a427]" /> {uiT('Orders', 'ऑर्डर')} (
                {orders.length})
              </h3>
              {orders.length > 0 && (
                <div className="flex items-center gap-3 text-base">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-base">
                            {(o.frameBrand || o.frame) && (
                              <>
                                <span className="text-th-secondary font-bold uppercase tracking-wider">
                                  {uiT('Frame', 'फ्रेम')}
                                </span>
                                <span className="text-th-text font-medium">
                                  {[o.frameBrand, o.frameModel, o.frameColor, o.frameSize]
                                    .filter(Boolean)
                                    .join(' / ') || o.frame}
                                  {o.framePrice ? (
                                    <span className="text-th-secondary"> ₹{o.framePrice}</span>
                                  ) : (
                                    ''
                                  )}
                                </span>
                            ...
                            {(o.lensBrand || o.lens) && (
                              <>
                                ...
                                <span className="text-th-text font-medium">
                                  {[o.lensBrand, o.lensType, o.lensIndex]
                                    .filter(Boolean)
                                    .join(' / ') || o.lens}
                                  {o.lensPrice ? (
                                    <span className="text-th-secondary"> ₹{o.lensPrice}</span>
                                  ) : (
                                    ''
                                  )}
                                </span>
                            ...
                            {o.coating && (
                              <>
                                ...
                                <span className="text-th-text font-medium">
                                  {o.coating}
                                  {o.coatingPrice ? (
                                    <span className="text-th-secondary"> ₹{o.coatingPrice}</span>
                                  ) : (
                                    ''
                                  )}
                                </span>
                          ...
                          <div className="flex items-center gap-3 mt-2 text-base text-th-secondary flex-wrap">
                            {o.quantity > 1 && (
                              <span className="flex items-center gap-0.5">
                                <Wallet size={10} /> {uiT('Qty', 'मात्रा')}: {o.quantity}
                              </span>
                            )}
                            {o.deliveryDate && (
                              <span className="flex items-center gap-0.5">
                                <Clock size={10} /> {uiT('Due', 'डिलीवरी')}:{' '}
                                {new Date(o.deliveryDate).toLocaleDateString('en-IN', {
                                  day: 'numeric',
                                  month: 'short',
                                })}
                            ...
                            {o.labAssigned && (
                              <span className="flex items-center gap-0.5">
                                <MapPinned size={10} /> {o.labAssigned}
                                {o.labExpectedDate
                                  ? ` (by ${new Date(o.labExpectedDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })})`
                                  : ''}
                              </span>
                            )}
                            {o.accessories?.length > 0 && (
                              <span className="text-th-secondary italic">
                                {o.accessories.join(', ')}
                              </span>
                            )}
                          </div>
        ...
        >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto scrollbar-none p-6 space-y-4">
              <div className="bg-th-elevated rounded-lg p-4">
                <h4 className="text-base font-bold uppercase tracking-wider text-th-text flex items-center gap-2 mb-3">
                  <User size={14} className="text-[#1ed760]" />{' '}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditForm({ ...editForm, alternateMobile: e.target.value })
                      }
                    />
                  </div>
                  <div>
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-th-border">
              <button
                onClick={() => setEditing(false)}
                className="text-base font-bold uppercase tracking-wider px-5 py-2 rounded-lg bg-th-elevated text-th-secondary hover:bg-th-hover hover:text-th-text transition-colors"
              >
              ...
              <button
                ...
                disabled={saving}
                className="flex items-center gap-1.5 text-base font-bold uppercase tracking-wider px-5 py-2 rounded-lg bg-[#1ed760] text-black hover:bg-[#1ed760]/90 active:scale-95 transition-transform disabled:opacity-50"
              >
          ...
          >
            <div className="sticky top-0 z-10 bg-th-surface border-b border-th-border px-6 py-4">
                ...
                <div className="flex items-center gap-2">
                  {!editingVisit && (
                    <button
                      ...
                      className="flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-2 rounded-lg bg-[#1ed760] text-black hover:bg-[#1ed760]/90 transition-colors"
                    >
                  ...
                  <button
                    onClick={() => setSelectedVisit(null)}
                    className="p-1.5 hover:bg-th-elevated rounded-lg transition-colors"
                  >
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                {linkedOrder && (
                  <span
                    className={`flex items-center gap-1 text-base font-bold uppercase tracking-wider ${
                      linkedOrder.status === 'Delivered' ? 'text-[#1ed760]' : 'text-[#e8a427]'
                    }`}
                  >
                    <ShoppingCart size={10} /> {uiT('Order', 'ऑर्डर')} ·{' '}
                ...
                {linkedBill && (
                  ...
                  >
                    ...
                    {(linkedBill.pendingAmount || 0) > 0 && (
                      <span> · ₹{linkedBill.pendingAmount} due</span>
                    )}
                  </span>
                )}
                {!linkedPrescription && !linkedOrder && !linkedBill && (
                  <span className="text-base text-th-secondary italic">
                    {uiT('No linked data', 'कोई लिंक डेटा नहीं')}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
                ...
                {editingVisit ? (
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditVisitForm({ ...editVisitForm, visitDate: e.target.value })
                          }
                        />
                      </div>
                      <div>
                        ...
                        <select
                          className={inputCls}
                          value={editVisitForm.visitType || 'new'}
                          onChange={(e) =>
                            setEditVisitForm({ ...editVisitForm, visitType: e.target.value })
                          }
                        >
                          ...
                          <option value="contact_lens">
                            {uiT('Contact Lens', 'कॉन्टैक्ट लेंस')}
                          </option>
                    ...
                    <div>
                      ...
                      <input
                        className={inputCls}
                        value={editVisitForm.doctorName || ''}
                        onChange={(e) =>
                          setEditVisitForm({ ...editVisitForm, doctorName: e.target.value })
                        }
                      />
                    </div>
                    <div>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-base font-bold uppercase tracking-wider px-4 py-1.5 rounded-lg bg-[#1ed760] text-black hover:bg-[#1ed760]/90 active:scale-95 transition-transform disabled:opacity-50"
                      >
                        <Save size={12} /> {savingVisit ? 'Saving...' : uiT('Save', 'सहेजें')}
                      </button>
                      <button
                        onClick={() => setEditingVisit(false)}
                        className="flex items-center gap-1.5 text-base font-bold uppercase tracking-wider px-4 py-1.5 rounded-lg bg-th-hover text-th-secondary hover:text-th-text transition-colors"
                      >
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-3 text-base text-th-secondary">
                        ...
                        {selectedVisit.doctorName && (
                          <span className="flex items-center gap-1">
                            <Stethoscope size={12} /> Dr. {selectedVisit.doctorName}
                          </span>
                        )}
                        <span className="flex items-center gap-1">
                          <Clock size={12} />{' '}
                      ...
                      {selectedVisit.remarks && (
                        <p className="text-base text-th-secondary mt-2 bg-th-hover rounded-md p-3">
                          {selectedVisit.remarks}
                        </p>
                      )}
                    </div>
              ...
              {linkedPrescription && (
                <div className="bg-th-elevated rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <Eye size={14} className="text-purple-400" />
                    <p className="text-base font-bold uppercase tracking-wider text-th-text">
                      {uiT('Prescription', 'प्रिस्क्रिप्शन')}
                    </p>
                    {!editingRx && (
                      <div className="flex items-center gap-1.5 ml-auto">
                        <span className="text-base text-th-secondary">
                          {new Date(linkedPrescription.createdAt).toLocaleDateString('en-IN', {
                            day: 'numeric',
                            month: 'short',
                            year: 'numeric',
                          })}
                        ...
                        <button
                          onClick={() => sendPrescriptionWhatsApp(linkedPrescription)}
                          disabled={sendingRx}
                          className="flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-1.5 rounded-lg bg-[#1ed760]/10 text-[#1ed760] hover:bg-[#1ed760]/20 transition-colors disabled:opacity-50"
                        >
                          {sendingRx ? (
                            <div className="animate-spin w-3 h-3 border-2 border-[#1ed760] border-t-transparent rounded-full" />
                          ) : (
                        ...
                        <button
                          onClick={openRxEdit}
                          className="flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-1.5 rounded-lg bg-th-hover text-th-secondary hover:text-th-text transition-colors"
                        >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {(['rightEye', 'leftEye'] as const).map((eye) => (
                          <div key={eye} className="bg-th-hover rounded-md p-3">
                            <p
                              className={`text-base font-bold uppercase tracking-wider mb-2 ${eye === 'rightEye' ? 'text-[#6ea8fe]' : 'text-[#e8a427]'}`}
                            >
                              {eye === 'rightEye'
                                ? uiT('Right Eye', 'दाहिनी आंख')
                                : uiT('Left Eye', 'बाईं आंख')}
                            ...
                            {(['dv', 'nv', 'pc'] as const).map((sub) => (
                              <div key={sub} className="flex items-center gap-1.5 mb-1.5">
                                <span className="text-base font-bold uppercase tracking-wider text-th-secondary w-7">
                                  {sub.toUpperCase()}
                                </span>
                                {(['sph', 'cyl', 'axis', 'va'] as const).map((k) => (
                                  <input
                                    ...
                                    className="w-16 bg-th-elevated text-th-text px-1.5 py-1 rounded text-base text-center outline-none focus:ring-1 focus:ring-[#1ed760]"
                                    value={editRxForm[eye]?.[sub]?.[k] ?? ''}
                                    onChange={(e) =>
                                      setEditRxForm({
                                        ...editRxForm,
                                        [eye]: {
                                          ...editRxForm[eye],
                                          [sub]: { ...editRxForm[eye]?.[sub], [k]: e.target.value },
                                        },
                                      })
                                    }
                                  />
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            className={inputCls}
                            value={editRxForm.pd || ''}
                            onChange={(e) => setEditRxForm({ ...editRxForm, pd: e.target.value })}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditRxForm({ ...editRxForm, notes: e.target.value })
                            }
                          />
                        </div>
                      </div>
                    ...
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="bg-th-hover rounded-md p-3">
                          ...
                          {linkedPrescription.rightEye?.dv && (
                            <EyeRow label="DV" data={linkedPrescription.rightEye.dv} />
                          )}
                          {linkedPrescription.rightEye?.nv && (
                            <EyeRow label="NV" data={linkedPrescription.rightEye.nv} />
                          )}
                          {linkedPrescription.rightEye?.pc && (
                            <EyeRow label="PC" data={linkedPrescription.rightEye.pc} />
                          )}
                          {!linkedPrescription.rightEye?.dv &&
                            !linkedPrescription.rightEye?.nv &&
                            !linkedPrescription.rightEye?.pc && (
                        ...
                        <div className="bg-th-hover rounded-md p-3">
                          ...
                          {linkedPrescription.leftEye?.dv && (
                            <EyeRow label="DV" data={linkedPrescription.leftEye.dv} />
                          )}
                          {linkedPrescription.leftEye?.nv && (
                            <EyeRow label="NV" data={linkedPrescription.leftEye.nv} />
                          )}
                          {linkedPrescription.leftEye?.pc && (
                            <EyeRow label="PC" data={linkedPrescription.leftEye.pc} />
                          )}
                          {!linkedPrescription.leftEye?.dv &&
                            !linkedPrescription.leftEye?.nv &&
                            !linkedPrescription.leftEye?.pc && (
                        ...
                        <div className="flex gap-4 mt-3 pt-3 border-t border-th-border text-base">
                          {linkedPrescription.pd && (
                            <p className="text-th-secondary">
                              <span className="font-medium">PD:</span> {linkedPrescription.pd}
                            </p>
                          )}
              ...
              {linkedOrder && (
                <div className="bg-th-elevated rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span
                      className={`text-sm font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ml-auto ${
                  ...
                  {editingOrder ? (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className={labelCls}>{uiT('Frame', 'फ्रेम')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.frame || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, frame: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Frame Brand', 'फ्रेम ब्रांड')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.frameBrand || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, frameBrand: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Frame Model', 'फ्रेम मॉडल')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.frameModel || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, frameModel: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Frame Color', 'फ्रेम रंग')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.frameColor || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, frameColor: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Frame Size', 'फ्रेम आकार')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.frameSize || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, frameSize: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Frame Price', 'फ्रेम मूल्य')}</label>
                          <input
                            type="number"
                            inputMode="numeric"
                            className={`${inputCls} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                            value={editOrderForm.framePrice ?? ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, framePrice: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Lens', 'लेंस')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.lens || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, lens: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Lens Brand', 'लेंस ब्रांड')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.lensBrand || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, lensBrand: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Lens Type', 'लेंस प्रकार')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.lensType || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, lensType: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Lens Index', 'लेंस इंडेक्स')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.lensIndex || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, lensIndex: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Lens Price', 'लेंस मूल्य')}</label>
                          <input
                            type="number"
                            inputMode="numeric"
                            className={`${inputCls} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                            value={editOrderForm.lensPrice ?? ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, lensPrice: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Coating', 'कोटिंग')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.coating || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, coating: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Coating Price', 'कोटिंग मूल्य')}</label>
                          <input
                            type="number"
                            inputMode="numeric"
                            className={`${inputCls} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                            value={editOrderForm.coatingPrice ?? ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, coatingPrice: e.target.value })
                            }
                          />
                        </div>
                      </div>
                      <div>
                        <label className={labelCls}>{uiT('Accessories', 'सहायक')}</label>
                        <input
                          className={inputCls}
                          placeholder="comma separated"
                          value={editOrderForm.accessories || ''}
                          onChange={(e) =>
                            setEditOrderForm({ ...editOrderForm, accessories: e.target.value })
                          }
                        />
                      </div>
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          <label className={labelCls}>{uiT('Qty', 'मात्रा')}</label>
                          <input
                            type="number"
                            inputMode="numeric"
                            className={`${inputCls} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                            value={editOrderForm.quantity ?? 1}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, quantity: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Delivery', 'डिलीवरी')}</label>
                          <input
                            type="date"
                            className={inputCls}
                        ...
                        <div>
                          ...
                          <select
                            className={inputCls}
                            value={editOrderForm.status || 'Draft'}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, status: e.target.value })
                            }
                          >
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          <label className={labelCls}>{uiT('Lab', 'लैब')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.labAssigned || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, labAssigned: e.target.value })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Lab Expected', 'लैब अपेक्षित')}</label>
                          <input
                            type="date"
                            className={inputCls}
                            value={editOrderForm.labExpectedDate || ''}
                            onChange={(e) =>
                              setEditOrderForm({
                                ...editOrderForm,
                                labExpectedDate: e.target.value,
                              })
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Lab Remarks', 'लैब टिप्पणी')}</label>
                          <input
                            className={inputCls}
                            value={editOrderForm.labRemarks || ''}
                            onChange={(e) =>
                              setEditOrderForm({ ...editOrderForm, labRemarks: e.target.value })
                            }
                          />
                        </div>
                      </div>
                    ...
                    <>
                      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-base">
                        {linkedOrder.frame && (
                          <>
                            <p className="text-th-secondary font-bold uppercase tracking-wider text-base">
                              {uiT('Frame', 'फ्रेम')}
                            </p>
                            <p className="font-semibold text-th-text text-right">
                              {[
                                linkedOrder.frameBrand,
                                linkedOrder.frameModel,
                                linkedOrder.frameColor,
                                linkedOrder.frameSize,
                              ]
                                .filter(Boolean)
                                .join(' / ') || linkedOrder.frame}
                        ...
                        {linkedOrder.lens && (
                          <>
                            ...
                            <p className="font-semibold text-th-text text-right">
                              {[linkedOrder.lensBrand, linkedOrder.lensType, linkedOrder.lensIndex]
                                .filter(Boolean)
                                .join(' / ') || linkedOrder.lens}
                        ...
                        {linkedOrder.coating && (
                          <>
                            ...
                            <p className="font-semibold text-th-text text-right">
                              {linkedOrder.coating}
                              {linkedOrder.coatingPrice ? ` (₹${linkedOrder.coatingPrice})` : ''}
                            </p>
                          </>
                        )}
                        {linkedOrder.accessories?.length > 0 && (
                          <>
                            <p className="text-th-secondary font-bold uppercase tracking-wider text-base">
                              {uiT('Accessories', 'सहायक')}
                            </p>
                            <p className="font-semibold text-th-text text-right">
                              {linkedOrder.accessories.join(', ')}
                            </p>
                          </>
                        )}
                        <p className="text-th-secondary font-bold uppercase tracking-wider text-base">
                        ...
                        {linkedOrder.deliveryDate && (
                          <>
                            ...
                            <p className="font-semibold text-th-text text-right">
                              {new Date(linkedOrder.deliveryDate).toLocaleDateString('en-IN', {
                                day: 'numeric',
                                month: 'short',
                                year: 'numeric',
                              })}
                        ...
                        {linkedOrder.labAssigned && (
                          <>
                            ...
                            <p className="font-semibold text-th-text text-right">
                              {linkedOrder.labAssigned}
                              {linkedOrder.labExpectedDate
                                ? ` (by ${new Date(linkedOrder.labExpectedDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })})`
                                : ''}
                            </p>
                          </>
                        )}
                      ...
                      {linkedOrder.labRemarks && (
                        <p className="text-base text-th-secondary mt-2 pt-2 border-t border-th-border">
                          {uiT('Lab Remarks', 'लैब टिप्पणी')}: {linkedOrder.labRemarks}
                        </p>
                      )}
                    </>
                  )}
              ...
              {linkedBill && (
                <div className="bg-th-elevated rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-base text-th-secondary ml-auto font-mono">
                      {linkedBill.billNumber}
                    </span>
                    {!editingBill && (
                      <button
                        onClick={openBillEdit}
                        className="flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-1.5 rounded-lg bg-th-hover text-th-secondary hover:text-th-text transition-colors"
                      >
                        <Edit3 size={11} /> {uiT('Edit', 'संपादित')}
                      </button>
                  ...
                  {editingBill ? (
                    <div className="space-y-3">
                        ...
                        {(editBillForm.items || []).map((it: any, idx: number) => (
                          <div key={idx} className="flex items-center gap-2">
                            <input
                              ...
                              placeholder={uiT('Item name', 'आइटम का नाम')}
                            ...
                            <input
                              type="number"
                              inputMode="numeric"
                              className={`${inputCls} w-16 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                              placeholder="Qty"
                              value={it.quantity}
                            ...
                            />
                            <span className="text-base font-semibold text-th-text w-20 text-right">
                              ₹
                              {((Number(it.quantity) || 1) * (Number(it.unitPrice) || 0)).toFixed(
                                0
                              )}
                            </span>
                      ...
                      <button
                        onClick={addBillItem}
                        className="flex items-center gap-1 text-base font-bold uppercase tracking-wider px-4 py-2 rounded-lg bg-th-hover text-th-secondary hover:text-th-text transition-colors"
                      >
                        <Plus size={11} /> {uiT('Add Item', 'आइटम जोड़ें')}
                      </button>
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          <label className={labelCls}>{uiT('Discount', 'छूट')}</label>
                          <input
                            type="number"
                            inputMode="numeric"
                            className={`${inputCls} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                            ...
                            onChange={(e) =>
                              setEditBillForm((f: any) => ({ ...f, discount: e.target.value }))
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('GST', 'जीएसटी')}</label>
                          <input
                            type="number"
                            inputMode="numeric"
                            className={`${inputCls} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                            ...
                            onChange={(e) =>
                              setEditBillForm((f: any) => ({ ...f, tax: e.target.value }))
                            }
                          />
                        </div>
                        <div>
                          <label className={labelCls}>{uiT('Paid', 'भुगतान')}</label>
                          <input
                            type="number"
                            inputMode="numeric"
                            className={`${inputCls} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                            value={editBillForm.advancePaid ?? ''}
                            onChange={(e) =>
                              setEditBillForm((f: any) => ({ ...f, advancePaid: e.target.value }))
                            }
                          />
                        </div>
                      </div>
                    ...
                    <>
                          ...
                          <table className="w-full text-base">
                            <thead>
                              <tr className="border-b border-th-border">
                                <th className="text-left py-2 px-2 font-bold uppercase tracking-wider text-th-secondary text-base">
                                  {uiT('Item', 'आइटम')}
                                </th>
                                <th className="text-center py-2 px-2 font-bold uppercase tracking-wider text-th-secondary text-base">
                                  Qty
                                </th>
                                <th className="text-right py-2 px-2 font-bold uppercase tracking-wider text-th-secondary text-base">
                                  {uiT('Price', 'मूल्य')}
                                </th>
                                <th className="text-right py-2 px-2 font-bold uppercase tracking-wider text-th-secondary text-base">
                                  {uiT('Total', 'कुल')}
                              ...
                              {linkedBill.items.map((it: any, i: number) => (
                                <tr key={it._id || it.description || i}>
                                  <td className="py-2 px-2 text-th-text">
                                    {it.description || uiT('Item', 'आइटम')}
                                  </td>
                                  <td className="py-2 px-2 text-center text-th-secondary">
                                    {it.quantity || 1}
                                  </td>
                                  <td className="py-2 px-2 text-right text-th-secondary">
                                    ₹{(it.unitPrice || 0).toFixed(0)}
                                  </td>
                                  <td className="py-2 px-2 text-right font-semibold text-th-text">
                                    ₹{((it.quantity || 1) * (it.unitPrice || 0)).toFixed(0)}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
                      ...
                      <div className="space-y-1.5 text-base">
                        <div className="flex justify-between text-th-secondary">
                          ...
                          <span>₹{(linkedBill.subtotal || 0).toFixed(0)}</span>
                        </div>
                        {linkedBill.discount ? (
                          <div className="flex justify-between text-red-400">
                            <span>{uiT('Discount', 'छूट')}</span>
                            <span>-₹{linkedBill.discount.toFixed(0)}</span>
                          </div>
                        ) : null}
                        {linkedBill.tax ? (
                          <div className="flex justify-between text-[#1ed760]">
                            <span>{uiT('GST', 'जीएसटी')}</span>
                            <span>+₹{linkedBill.tax.toFixed(0)}</span>
                          </div>
                        ) : null}
                        <div className="flex justify-between font-bold text-th-text pt-1.5 border-t border-th-border">
                          <span>{uiT('Total', 'कुल')}</span>
                          <span>₹{(linkedBill.totalAmount || 0).toFixed(0)}</span>
                        </div>
                            ...
                            {editingBillAdvance ? (
                              <div className="flex items-center gap-1">
                                <input
                                  ...
                                  min="0"
                                  step="0.01"
                                  className="w-24 bg-th-hover text-th-text px-2 py-1 rounded text-base text-right outline-none focus:ring-1 focus:ring-[#1ed760]"
                                  value={editBillAdvanceAmount}
                                  onFocus={(e) => e.target.select()}
                                  onChange={(e) => setEditBillAdvanceAmount(Number(e.target.value))}
                                />
                                <button
                                  onClick={handleBillAdvanceSave}
                                  disabled={savingBillAdvance}
                                  className="p-1 hover:bg-[#1ed760]/10 rounded text-[#1ed760] transition-colors"
                                >
                                  {savingBillAdvance ? (
                                    <div className="animate-spin w-3 h-3 border-2 border-[#1ed760] border-t-transparent rounded-full" />
                                  ) : (
                                ...
                                <button
                                  onClick={() => setEditingBillAdvance(false)}
                                  className="p-1 hover:bg-red-500/10 rounded text-red-400 transition-colors"
                                >
                                  <X size={12} />
                                </button>
                              ...
                              <div className="flex items-center gap-1.5">
                                <span className="font-semibold">
                                  ₹{linkedBill.advancePaid.toFixed(0)}
                                </span>
                                <button
                                  onClick={() => {
                                    setEditBillAdvanceAmount(linkedBill.advancePaid || 0);
                                    setEditingBillAdvance(true);
                                  }}
                  ...
                  <div className="mt-3 pt-3 border-t border-th-border">
                    <button
                      onClick={() => sendWhatsApp(customer?.mobile, linkedBill)}
                      disabled={sendingBill}
                    ...
                    >
                      {sendingBill ? (
                        <div className="animate-spin w-3.5 h-3.5 border-2 border-[#1ed760] border-t-transparent rounded-full" />
                      ) : (
                        <MessageCircle size={13} />
                      )}{' '}
                      {uiT('Send on WhatsApp', 'व्हाट्सऐप पर भेजें')}
                    </button>
                  </div>
                </div>
              )}
              ...
              {!linkedPrescription && !linkedOrder && !linkedBill && (
                <p className="text-base text-th-secondary py-8 text-center">
                  {uiT('No linked data for this visit.', 'इस विज़िट के लिए कोई लिंक डेटा नहीं।')}
                </p>
              )}
            </div>
          </div>
