...
const v = <T,>(val: T | null | undefined, fallback: T | string = '—'): T | string =>
  val ?? fallback;
...
function MetricCard({
  ...
  return (
    <ShineCard className="flex flex-col items-center text-center bg-th-surface rounded-xl px-2 sm:px-3 py-3 sm:py-4 h-full active:scale-95 shadow-md hover:shadow-lg hover:-translate-y-0.5 cursor-default transition-all duration-200 group">
      ...
      <p className="text-[11px] sm:text-[12px] font-semibold text-th-secondary mt-1 sm:mt-1.5 leading-tight line-clamp-2">
        {label}
      </p>
      {subtitle && (
        <p className="text-[10px] sm:text-[11px] text-th-muted mt-0.5 leading-tight line-clamp-1">
          {subtitle}
        </p>
      )}
    </ShineCard>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 text-left">
        <span className="block text-[13px] sm:text-[15px] font-bold text-th-text truncate leading-tight">
          {label}
        </span>
        <span className="block text-[11px] sm:text-[13px] text-th-secondary truncate leading-tight mt-0.5 hidden sm:block">
          {subtitle}
        </span>
      </div>
  ...
  return (
    <div className="flex items-center justify-between mb-3 sm:mb-4">
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {count !== undefined && (
          <span className="text-[13px] sm:text-[16px] font-medium text-th-secondary bg-th-elevated px-2 sm:px-2.5 py-0.5 rounded-lg">
            {count}
          </span>
        )}
      </div>
      {action && (
        <button
          onClick={action}
          aria-label={actionLabel || 'View all'}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[11px] sm:text-[13px] font-bold px-1.5 sm:px-2 py-0.5 rounded-md whitespace-nowrap uppercase tracking-wider flex-shrink-0 ${styles[status] || 'bg-th-elevated text-th-secondary'}`}
    >
      {status}
    </span>
  );
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 sm:py-14 text-center px-4 sm:px-6">
      ...
      <p className="text-[14px] sm:text-[18px] text-th-secondary mt-1 mb-4 sm:mb-5 max-w-xs">
        {description}
      </p>
...
function SegmentedControl({
  ...
  return (
    <div
      className={`flex rounded-lg bg-th-elevated gap-0.5 border border-th-border ${compact ? 'p-0.5' : 'p-1'}`}
    >
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onChange(isActive ? 'pending' : opt.value);
            }}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 sm:gap-3 h-full">
        <div
          className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${bgMap[color] || bgMap.blue}`}
        >
          <Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${iconMap[color] || iconMap.blue}`} />
        </div>
        <div className="flex-1 min-w-0">
          <p
            className={`text-[14px] sm:text-[17px] font-medium ${textMap[color] || textMap.blue} uppercase tracking-wider leading-tight truncate`}
          >
            {label}
          </p>
          <p
        ...
        {action && (
          <button
            ...
            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-[13px] sm:text-[16px] font-bold transition-all active:scale-95 ${textMap[color] || textMap.blue} bg-th-card hover:bg-th-elevated uppercase tracking-wider flex-shrink-0 whitespace-nowrap`}
          >
            {actionLabel || 'View'}
          </button>
        )}
...
export default function Dashboard() {
  ...
  const saveEdit = useCallback(
    async (id: string) => {
      ...
      if (!task) {
        ...
        setEditText(prev.notes ? `${prev.task}\n${prev.notes}` : prev.task);
        return;
      }
      const updated: Todo = { ...prev, task, notes };
      setTodos((p) => p.map((t) => (t._id === id ? updated : t)));
      const res = await api.patch<Todo>(`/api/workspace/todos/${id}`, {
        task,
  ...
  const classifyEye = useCallback(
    async (id: string, eye: 'right' | 'left', status: string) => {
  ...
  const sendDemand = useCallback(
    async (type: 'buy' | 'order') => {
      ...
      if (res.success) {
        ...
        if (d.sent) {
          toast.success(
            type === 'buy' ? 'Purchase list sent to WhatsApp!' : 'Lab order list sent to WhatsApp!'
          );
        } else if (d.waConnected === false) {
          toast.error('WhatsApp not connected. Scan QR code on WhatsApp page.');
        } else if (d.queued) {
          toast.info(
            `${type === 'buy' ? 'Purchase' : 'Lab Order'} list queued — will send when connected`
          );
        } else {
  ...
  const greeting =
    hour < 12
      ? uiT('Good morning', 'शुभ प्रभात')
      : hour < 17
        ? uiT('Good afternoon', 'नमस्ते')
        : uiT('Good evening', 'शुभ संध्या');
  ...
  const totalStock = (ss: { shop?: number; warehouse?: number } | null | undefined) =>
    (ss?.shop ?? 0) + (ss?.warehouse ?? 0);
      ...
      <div className="min-w-0">
        <h1 className="text-[20px] sm:text-[28px] font-bold text-th-text tracking-tight truncate">
          ...
          <span className="text-[#1ed760]">
            {'Mr ' + (currentBranch?.settings?.ownerName || user?.name || user?.username || '')}
          </span>
        </h1>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 flex-shrink-0">
          <div className="hidden sm:flex items-center gap-1.5 text-[11px] sm:text-[12px] text-th-muted mr-0.5">
          ...
          <button
            onClick={() => refetch()}
            aria-label={uiT('Refresh', 'रीफ्रेश')}
            className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-th-elevated text-th-secondary hover:text-th-text hover:bg-th-card flex items-center justify-center transition-all active:scale-95"
            title={uiT('Refresh data', 'डेटा रीफ्रेश करें')}
          >
            <RefreshCw className="w-3.5 h-3.5" />
          </button>
          <button
            onClick={() => navigate('/workspace')}
          aria-label={uiT('New Sale', 'नई बिक्री')}
  ...
  const renderQuickActions = () => (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3">
        <QuickActionCard
          icon={ShoppingCart}
          label={uiT('New Sale', 'नई बिक्री')}
          subtitle={uiT('Create a new order', 'नया ऑर्डर बनाएं')}
          onClick={() => navigate('/workspace')}
          color="#6366f1"
        />
        <QuickActionCard
          icon={UserPlus}
          label={uiT('Customers', 'ग्राहक')}
          subtitle={uiT('Manage your clients', 'अपने ग्राहकों का प्रबंधन करें')}
          onClick={() => navigate('/customers')}
          color="#10b981"
        />
        <QuickActionCard
          icon={Boxes}
          label={uiT('Inventory', 'इन्वेंट्री')}
          subtitle={uiT('Track stock & lenses', 'स्टॉक और लेंस ट्रैक करें')}
          onClick={() => navigate('/inventory')}
          color="#f59e0b"
        />
        <QuickActionCard
          icon={BarChart3}
          label={uiT('Reports', 'रिपोर्ट')}
          subtitle={uiT('View business insights', 'व्यापार जानकारी देखें')}
        ...
        <QuickActionCard
          icon={Receipt}
          label={uiT('Bills', 'बिल')}
          subtitle={uiT('Manage pending payments', 'लंबित भुगतान प्रबंधित करें')}
        ...
        <QuickActionCard
          ...
          subtitle={uiT('View all orders', 'सभी ऑर्डर देखें')}
          onClick={() => navigate('/orders')}
          color="#06b6d4"
        />
        <QuickActionCard
          icon={ScanLine}
          label={uiT('Scanner', 'स्कैनर')}
          subtitle={uiT('Scan product barcodes', 'उत्पाद बारकोड स्कैन करें')}
          onClick={() => setShowScanner(true)}
          color="#f97316"
        />
        <QuickActionCard
          icon={MessageSquare}
          label="WhatsApp"
          subtitle={uiT('Send messages & PDFs', 'संदेश और PDF भेजें')}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 sm:gap-3">
        <MetricCard
          ...
          subtitle={uiT('vs last week', 'पिछले सप्ताह की तुलना में')}
        />
        <MetricCard
          label={uiT("Today's Collection", 'आज का संग्रह')}
          value={`₹${(d.todayCollection || 0).toLocaleString()}`}
          icon={IndianRupee}
          color="#6366f1"
          subtitle={uiT('today', 'आज')}
        />
        <MetricCard
          label={uiT("Today's Orders", 'आज के ऑर्डर')}
          value={d.todayOrders}
          icon={ShoppingBag}
          color="#8b5cf6"
          subtitle={d.weekOrders ? `${d.weekOrders} ${uiT('this week', 'इस सप्ताह')}` : undefined}
        />
        <MetricCard
          label={uiT('Pending Bill Amount', 'लंबित बिल राशि')}
          value={d.pendingBills.length}
          icon={Receipt}
          color="#ef4444"
          subtitle={`₹${(d.pendingPayments || 0).toLocaleString()} due`}
        />
        <MetricCard
          label={uiT('Ready for Pickup', 'पिकअप के लिए तैयार')}
          value={d.readyDeliveries ?? 0}
          icon={PackageCheck}
          color="#06b6d4"
          subtitle={uiT('awaiting collection', 'संग्रह की प्रतीक्षा में')}
        />
        <MetricCard
          label={uiT('New Customers', 'नए ग्राहक')}
          value={d.newCustomersToday ?? 0}
          icon={Users}
          color="#10b981"
          subtitle={uiT('joined today', 'आज जुड़े')}
        />
        <MetricCard
          label={uiT('Low Stock Items', 'कम स्टॉक आइटम')}
          value={d.lowStock ?? 0}
          icon={AlertTriangle}
          color="#f59e0b"
          subtitle={uiT('items need restock', 'आइटम को रीस्टॉक की आवश्यकता')}
        />
        <MetricCard
          label={uiT('Total Inventory', 'कुल इन्वेंट्री')}
          value={d.counts.inventory}
          icon={Boxes}
          color="#f472b6"
          subtitle={uiT('total SKUs', 'कुल SKU')}
        />
      </div>
    </div>
  );
  ...
  const renderNeedsAttention = () => {
    ...
    if (d.pendingBills.length > 0)
      items.push({
        ...
        actionLabel: uiT('Collect', 'वसूलें'),
        onClick: () => navigate('/bills'),
      });
    if ((d.lowStock ?? 0) > 0)
      items.push({
    ...
    if (draftOrders.length > 0)
      items.push({
        icon: FileText,
        label: uiT('Draft Orders', 'ड्राफ्ट ऑर्डर'),
        value: draftOrders.length,
        color: 'yellow',
        action: undefined,
        onClick: undefined,
      });
    if (d.todayDeliveries.length > 0)
      items.push({
        icon: Truck,
        label: uiT("Today's Deliveries", 'आज की डिलीवरी'),
        value: d.todayDeliveries.length,
        color: 'blue',
      ...
      <div className="bg-th-surface rounded-xl overflow-hidden shadow-lg h-full flex flex-col">
        <div className="px-3 sm:px-5 py-3 sm:py-4 border-b border-th-card">
          <div className="flex items-center justify-between mb-2 sm:mb-2.5">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              {selectedOrders.size > 0 && (
                <span className="text-[13px] sm:text-[16px] font-bold text-[#1ed760] bg-[#1ed760]/10 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg">
                  {selectedOrders.size} {uiT('selected', 'चयनित')}
                </span>
              )}
              <button
                onClick={() => sendDemand('buy')}
                disabled={sendingDemand !== null || selectedOrders.size === 0}
                aria-label={uiT('Buy lenses', 'लेंस खरीदें')}
              ...
              <button
                onClick={() => sendDemand('order')}
                disabled={sendingDemand !== null || selectedOrders.size === 0}
                aria-label={uiT('Order lenses', 'लेंस ऑर्डर करें')}
          ...
          <label className="flex items-center gap-2 cursor-pointer text-[13px] sm:text-[16px] font-medium text-th-secondary hover:text-th-text transition-colors">
            <input
              type="checkbox"
              checked={selectedOrders.size === draftOrders.length && draftOrders.length > 0}
        ...
        {draftOrders.length === 0 ? (
          <EmptyState
            icon={PackageCheck}
            title={uiT('All clear!', 'सब साफ!')}
            description={uiT(
              'No draft orders pending lens classification.',
              'लेंस वर्गीकरण के लिए कोई ड्राफ्ट ऑर्डर लंबित नहीं।'
            )}
            actionLabel={uiT('Create New Order', 'नया ऑर्डर बनाएं')}
          ...
          <div className="divide-y divide-th-border max-h-[440px] overflow-y-auto scrollbar-none">
            {draftOrders.map((o) => {
              ...
              const rRx = compactRx(
                rDV as Record<string, unknown> | undefined,
                rNV as Record<string, unknown> | undefined
              );
              const lRx = compactRx(
                lDV as Record<string, unknown> | undefined,
                lNV as Record<string, unknown> | undefined
              );
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2.5 sm:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        <p className="text-[15px] sm:text-[18px] font-bold text-th-text truncate">
                          {cName || '—'}
                        </p>
                        {!!cMobile && (
                          <span className="text-[13px] sm:text-[16px] text-th-muted hidden sm:inline">
                            {maskPhone(cMobile)}
                          </span>
                        )}
                        <span className="text-[13px] sm:text-[16px] text-th-muted hidden sm:inline">
                        ...
                        <span className="text-[13px] sm:text-[16px] text-th-muted">
                          {o.createdAt ? formatTimeAgo(o.createdAt, uiT) : ''}
                        </span>
                      </div>
                      <div className="flex items-center gap-1 sm:gap-1.5 mt-0.5 flex-wrap">
                        {!!o.frameBrand && (
                          <span className="text-[13px] sm:text-[15px] text-th-secondary bg-th-elevated px-1.5 sm:px-2 py-0.5 rounded-md">
                            {o.frameBrand?.trim()}
                          </span>
                        )}
                        {!!o.lensBrand && (
                        ...
                        {!!o.lensType && (
                          <span className="text-[13px] sm:text-[15px] text-th-muted bg-th-elevated px-1.5 sm:px-2 py-0.5 rounded-md">
                            {o.lensType}
                          </span>
                        )}
                        {!!o.lensIndex && (
                  ...
                  <div className=" mt-2 sm:mt-3 space-y-1.5 sm:space-y-2">
                    {/* R Eye */}
                    <div className="flex items-center gap-2 sm:gap-3">
                      <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-md bg-[#1ed760]/10 text-[#1ed760] text-[13px] sm:text-[15px] font-extrabold flex items-center justify-center flex-shrink-0">
                        R
                      </span>
                      ...
                      <SegmentedControl
                        ...
                        options={[
                          {
                            label: uiT('Stock', 'स्टॉक'),
                            value: 'stock',
                            color: 'bg-[#1ed760] text-black ',
                          },
                          {
                            label: uiT('Buy', 'खरೀदें'),
                            value: 'buy',
  ...
  const renderRecentOrders = () => (
    <div className="bg-th-surface rounded-xl overflow-hidden shadow-lg h-full flex flex-col">
      <div className="px-3 sm:px-5 py-3 sm:py-4 border-b border-th-border">
        <SectionHeader
          title={uiT("Today's Orders", 'आज के ऑर्डर')}
          count={d.todayOrders}
          action={() => navigate('/orders')}
          actionLabel={uiT('View all', 'सभी देखें')}
        />
      </div>
      <div className="divide-y divide-th-border max-h-[465px] overflow-y-auto scrollbar-none flex-1">
        {d.recentOrders.length === 0 ? (
          <EmptyState
            icon={ClipboardList}
            title={uiT('No orders today', 'आज कोई ऑर्डर नहीं')}
            description={uiT("Today's orders will appear here.", 'आज के ऑर्डर यहां दिखाई देंगे।')}
            actionLabel={uiT('New Order', 'नया ऑर्डर')}
            onAction={() => navigate('/workspace')}
          />
        ) : (
          d.recentOrders.map((o, idx) => {
            ...
            if (rx?.rightEye?.dv) {
              ...
              const parts = [
                r.sph != null ? `SPH ${r.sph}` : '',
                r.cyl != null ? `CYL ${r.cyl}` : '',
                r.axis != null ? `AX ${r.axis}` : '',
              ].filter(Boolean);
            ...
            if (rx?.leftEye?.dv) {
              ...
              const parts = [
                l.sph != null ? `SPH ${l.sph}` : '',
                l.cyl != null ? `CYL ${l.cyl}` : '',
                l.axis != null ? `AX ${l.axis}` : '',
              ].filter(Boolean);
            ...
            return (
              <div
                key={o._id || idx}
                className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-5 py-3 sm:py-3.5 hover:bg-th-card transition-all cursor-pointer"
                onClick={() => navigate(`/workspace?order=${o._id}`)}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => e.key === 'Enter' && navigate(`/workspace?order=${o._id}`)}
              >
                <div className="relative flex-shrink-0">
                  <UserAvatar
                    name={cName}
                    className="w-8 h-8 sm:w-10 sm:h-10 text-[10px] sm:text-sm"
                  />
                  <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-th-surface flex items-center justify-center">
                    <div
                      className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${o.status === 'Delivered' ? 'bg-[#1ed760]' : o.status === 'Draft' ? 'bg-th-muted' : o.status === 'Ordered' ? 'bg-[#a78bfa]' : o.status === 'Ready' ? 'bg-[#3498db]' : 'bg-[#f59e0b]'}`}
                    />
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] sm:text-[17px] font-semibold text-th-text truncate">
                    {cName}
                  </p>
  ...
  const renderPendingBills = () => (
    <div className="bg-th-surface rounded-xl overflow-hidden shadow-lg flex flex-col h-full">
      <div className="px-3 sm:px-5 py-3 sm:py-4 border-b border-th-card">
        <SectionHeader
          title={uiT('Pending Bill Amount', 'लंबित बिल राशि')}
          count={d.pendingBills.length}
          action={() => navigate('/bills')}
          actionLabel={uiT('View all', 'सभी देखें')}
        />
      </div>
      <div className="divide-y divide-th-card max-h-[380px] overflow-y-auto scrollbar-none flex-1">
        {d.pendingBills.length === 0 ? (
          <EmptyState
            icon={IndianRupee}
            title={uiT('All bills cleared', 'सभी बिल चुकता')}
            description={uiT('No pending bills to collect.', 'कोई लंबित बिल नहीं।')}
          />
        ) : (
          d.pendingBills.map((b, idx) => {
            ...
            return (
              <div
                key={b._id || idx}
                className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-5 py-3 sm:py-4 hover:bg-th-card transition-all"
              >
                <UserAvatar
                  name={cName}
                  className="w-8 h-8 sm:w-10 sm:h-10 text-[10px] sm:text-sm"
                ...
                <div className="flex items-center gap-2 sm:gap-2.5 flex-shrink-0">
                  <p className="text-[14px] sm:text-[17px] font-bold text-[#e74c3c] whitespace-nowrap">
                    ₹{(b.pendingAmount || 0).toLocaleString()}
                  </p>
                  <button
                    onClick={() => navigate(`/collect?billId=${b._id}`)}
                    aria-label={uiT('Collect payment', 'भुगतान वसूलें')}
  ...
  const renderDeliveries = () => {
    ...
    const renderRow = (item: Order, idx: number) => {
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12px] sm:text-[14px] text-th-secondary mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
                ...
                {!o.frameBrand && !o.lensBrand
                  ? o.createdAt
                    ? formatTimeAgo(o.createdAt, uiT)
                    : ''
                  : ''}
              </p>
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
              <span className="text-[14px] sm:text-[17px] font-bold text-th-text whitespace-nowrap">
      ...
      return (
        <div
          key={item._id || idx}
          className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-5 py-2.5 sm:py-3 hover:bg-th-card transition-all"
        >
          <div className="relative flex-shrink-0">
            <UserAvatar name={cName} className="w-8 h-8 sm:w-10 sm:h-10 text-[10px] sm:text-sm" />
            <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-th-surface flex items-center justify-center">
              <div
                className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${String(item.status) === 'In Transit' ? 'bg-[#f59e0b]' : item.status === 'Ready' ? 'bg-[#3498db]' : item.status === 'Delivered' ? 'bg-[#1ed760]' : item.status === 'Ordered' ? 'bg-[#a78bfa]' : 'bg-[#e74c3c]'}`}
              />
            </span>
          </div>
          <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            <button
              onClick={() => navigate(`/delivery?order=${item._id}`)}
              aria-label={uiT('Deliver', 'डिलीवर')}
      ...
      <div className="bg-th-surface rounded-xl overflow-hidden shadow-lg border border-th-border flex flex-col h-full">
        <div className="px-3 sm:px-5 pt-3 sm:pt-3.5 pb-0 border-b border-th-card">
          <div className="flex items-center justify-between gap-2 mb-2 sm:mb-2.5">
            ...
            <div className="flex items-center gap-1 sm:gap-1.5">
              {tabs.map((t) => (
                <button
                  key={t.value}
                  onClick={() => setDeliveriesTab(t.value)}
                  aria-label={t.label}
                  className={`px-2 sm:px-2.5 py-1 rounded-lg text-[11px] sm:text-[13px] font-bold uppercase tracking-wider transition-all active:scale-95 flex items-center gap-1 ${
                    deliveriesTab === t.value
                ...
                >
                  {t.label}
                  <span
                    className={`px-1.5 rounded-md text-[10px] sm:text-[11px] ${deliveriesTab === t.value ? 'bg-black/20' : 'bg-th-card'}`}
                  >
                    {t.count}
                  </span>
                </button>
          ...
          <div className="flex items-center justify-end pb-2 sm:pb-2.5">
            <button
              onClick={() => navigate('/delivery')}
              aria-label={uiT('View all', 'सभी देखें')}
        ...
        <div className="divide-y divide-th-card max-h-[380px] overflow-y-auto scrollbar-none flex-1">
          {list.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-6 sm:py-8 text-center px-4">
              ...
              <p className="text-[14px] sm:text-[16px] font-semibold text-th-text">
                {emptyState.title}
              </p>
              <p className="text-[12px] sm:text-[13px] text-th-secondary mt-0.5 max-w-xs">
                {emptyState.desc}
              </p>
            </div>
          ) : (
    ...
    <div className="bg-th-surface rounded-xl p-3 sm:p-5 shadow-lg flex flex-col">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <h3 className="text-[17px] sm:text-[20px] font-bold text-th-text uppercase tracking-wider">
            {uiT('To-Do', 'कार्य सूची')}
          </h3>
        </div>
        <span className="text-[13px] sm:text-[16px] font-bold text-th-secondary bg-th-elevated px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg">
          {activeTodos.length} {uiT('pending', 'बाकी')}
        </span>
      </div>
      ...
      <div className="mb-3 sm:mb-4">
        ...
        <div className="flex items-center justify-between gap-2 mt-1.5 sm:mt-2">
          <span className="text-[11px] sm:text-[12px] text-th-muted">
            {uiT('Enter = add · Shift+Enter = new line', 'Enter = जोड़ें · Shift+Enter = नई लाइन')}
          </span>
          <button
            onClick={addTodo}
            disabled={!newTodo.trim()}
            aria-label={uiT('Add task', 'कार्य जोड़ें')}
      ...
      <div className="space-y-1.5 max-h-[260px] overflow-y-auto scrollbar-none pr-1 flex-1">
        {todos.length === 0 ? (
          <EmptyState
            icon={CheckSquare}
            title={uiT('No tasks yet', 'अभी तक कोई कार्य नहीं')}
            description={uiT(
              'Add a task above to get started.',
              'शुरू करने के लिए ऊपर एक कार्य जोड़ें।'
            )}
          />
        ) : (
          [...activeTodos, ...doneTodos].map((t) =>
            editingId === t._id ? (
              <div
                key={t._id}
                className="bg-th-elevated border border-[#1ed760]/30 rounded-xl p-2.5 sm:p-3"
              >
                <AutoGrowTextarea
                  value={editText}
              ...
              <div
                key={t._id}
                className={`flex items-start gap-2.5 sm:gap-3 py-2 sm:py-2.5 px-2 sm:px-3 rounded-lg hover:bg-th-card group transition-all ${t.done ? 'opacity-40' : ''}`}
              >
                <button
                  onClick={() => toggleTodo(t._id, t.done)}
                  aria-label={
                    t.done
                      ? uiT('Mark incomplete', 'अपूर्ण चिह्नित करें')
                      : uiT('Mark complete', 'पूर्ण चिह्नित करें')
                  }
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-[15px] sm:text-[18px] leading-snug break-words ${t.done ? 'line-through text-th-muted' : 'text-th-secondary'}`}
                  >
                    {t.task}
                  </p>
                  {t.notes && (
                    <p className="text-[13px] sm:text-[15px] text-th-muted whitespace-pre-wrap break-words mt-0.5 leading-relaxed">
                      {t.notes}
                    </p>
                  )}
                </div>
    ...
    return (
      <div className="bg-th-surface rounded-xl p-3 sm:p-5 shadow-lg flex flex-col h-full">
        <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
          ...
          <h3 className="text-[17px] sm:text-[20px] font-bold text-th-text uppercase tracking-wider">
            {uiT("Today's Payments", 'आज का भुगतान')}
          </h3>
        </div>
        <div className="space-y-2 sm:space-y-2.5">
          {todayPayments.map((p, idx) => {
            ...
            return (
              <div
                key={`${p.mode}-${idx}`}
                className="flex items-center gap-2.5 sm:gap-3 bg-th-elevated rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 transition-all hover:bg-th-card"
              >
                <div
                  className="w-7 h-7 sm:w-9 sm:h-9 rounded-md sm:rounded-lg flex items-center justify-center flex-shrink-0"
                  style={{ backgroundColor: `${paymentModeColors[p.mode] || '#1ed760'}15` }}
                >
                  <Icon
                    className="w-3.5 h-3.5 sm:w-4 sm:h-4"
                    style={{ color: paymentModeColors[p.mode] || '#1ed760' }}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] sm:text-[18px] font-semibold text-th-text">{p.mode}</p>
                  <p className="text-[12px] sm:text-[16px] text-th-secondary">
                    {p.count} transaction{p.count !== 1 ? 's' : ''}
                  </p>
                </div>
                <p className="text-[15px] sm:text-[20px] font-bold text-th-text flex-shrink-0">
  ...
  const renderSummary = () => {
    const rows: { label: string; value: string | number; color?: string }[] = [
      ...
      {
        label: uiT('Total Deliveries', 'कुल डिलीवरी'),
        value: d.counts.deliveries,
        color: '#2dd4bf',
      },
      { label: uiT('Total Visits', 'कुल विज़िट'), value: d.counts.visits, color: '#fb923c' },
      {
        label: uiT('Today Sales', 'आज की बिक्री'),
        value: `₹${(d.todaySales || 0).toLocaleString()}`,
        color: '#34d399',
      },
      {
        label: uiT('Week Sales', 'सप्ताह की बिक्री'),
        value: `₹${(d.weekSales || 0).toLocaleString()}`,
        color: '#34d399',
      },
      {
        label: uiT('Month Sales', 'महीने की बिक्री'),
        value: `₹${(d.monthSales || 0).toLocaleString()}`,
        color: '#34d399',
      },
      {
        label: uiT('Today Collection', 'आज का संग्रह'),
        value: `₹${(d.todayCollection || 0).toLocaleString()}`,
        color: '#60a5fa',
      },
      {
        label: uiT('Pending Payments', 'बाकी भुगतान'),
      ...
      {
        label: uiT('New Customers Today', 'आज नए ग्राहक'),
        value: d.newCustomersToday ?? 0,
        color: '#60a5fa',
      },
      ...
      {
        label: uiT('Today Deliveries', 'आज की डिलीवरी'),
        value: d.todayDeliveries.length,
        color: '#2dd4bf',
      },
    ];
      ...
      <details className="group bg-th-surface rounded-xl overflow-hidden shadow-lg">
        <summary className="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4 cursor-pointer list-none hover:bg-th-card transition-colors">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span className="text-[15px] sm:text-[18px] font-bold text-th-text uppercase tracking-wider">
              {uiT('Summary', 'सारांश')}
            </span>
          </div>
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 sm:gap-x-6 gap-y-0.5 pt-3 border-t border-th-card">
            {rows.map((r) => (
              <div
                key={r.label}
                className="flex items-center justify-between py-1.5 border-b border-th-elevated last:border-0"
              >
                <span className="text-[14px] sm:text-[18px] text-th-secondary">{r.label}</span>
                <span
                  className="text-[14px] sm:text-[18px] font-bold text-th-text"
      ...
      {showScanner && (
        ...
        >
          <div className="relative w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setShowScanner(false)}
              className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-th-card flex items-center justify-center text-th-secondary hover:text-th-text z-10 transition-all active:scale-95"
