...
export default function Pickup() {
  ...
  useEffect(() => {
    ...
    api.get<{ status: string }>('/api/whatsapp/status').then((d) => {
      if (d.success && d.data) setWaStatus(d.data.status);
    });
    fetchReadyOrders();
  }, []);
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        const custRes = await api.get<Customer>(
          '/api/customers/' + (typeof o.customerId === 'object' ? o.customerId._id : o.customerId)
        );
        if (custRes.success && custRes.data) {
          ...
          if (ordersRes.success)
            setOrders(
              ...
              ).filter((o2: any) => o2.status === 'Ready')
            );
          let custBills: Bill[] = [];
          if (billsRes.success) {
            custBills = (
              (billsRes.data as any)?.data ||
              ((Array.isArray(billsRes.data) ? billsRes.data : []) as Bill[])
            ).filter((b: Bill) => {
  ...
  async function searchCustomer() {
    ...
    try {
      ...
      if (res.success && custList.length > 0) {
        ...
        setCustomers([]);
        setMessage(uiT('No customer found with this number', 'इस नंबर से कोई ग्राहक नहीं मिला'));
      }
    } finally {
      setIsLoading(false);
  ...
  async function syncBillForOrder(o: Order, custBills?: Bill[]): Promise<Bill | null> {
    ...
    if (!b) {
      ...
      if ((o as any).frame)
        items.push({
          description: (o as any).frameBrand
            ? `${(o as any).frame} (${o.frameBrand})`
            : (o as any).frame,
          qty: 1,
          price: (o as any).framePrice || 0,
        });
      if (o.lensBrand)
        items.push({
          description: o.lensBrand ? `${o.lensBrand} ${o.lensType || ''}` : o.lensBrand,
          qty: 1,
          price: (o as any).lensPrice || 0,
        });
      if (o.coating)
        items.push({ description: o.coating, qty: 1, price: (o as any).coatingPrice || 0 });
      if ((o as any).accessories?.length) {
        (o as any).accessories.forEach((a: string) =>
          items.push({ description: a, qty: 1, price: 0 })
        );
      }
  ...
  async function selectCustomer(c: Customer) {
    ...
    try {
      ...
      if (ordersRes.success) {
        ...
        setOrders(pending);
        if (pending.length === 0)
          setMessage(uiT('No orders ready for pickup', 'पिकअप के लिए कोई ऑर्डर तैयार नहीं'));
      }
      if (billsRes.success) {
        const custBills = (
          (billsRes.data as any)?.data ||
  ...
  function addBillItem() {
    setBillItems([...billItems, { description: '', qty: 1, price: 0 }]);
  }
  ...
  async function handleCreateBill() {
    ...
    if (items.length === 0) {
      setMessage(
        uiT(
          'Add at least one item with description and price',
          'कम से कम एक आइटम विवरण और मूल्य के साथ जोड़ें'
        )
      );
      return;
    }
    ...
    if (res.success) {
      ...
      setMessage(res.message || uiT('Failed to create bill', 'बिल बनाने में विफल'));
    }
  }
  ...
  async function handleDeliver() {
    ...
    if (isDelivered && collectAmount > 0) {
      ...
      if (res.success) {
        setMessage(
          `✓ ${uiT('Payment collected successfully!', 'भुगतान सफलतापूर्वक एकत्रित हुआ!')}`
        );
        globalToast.success(uiT('Payment collected', 'भुगतान एकत्रित हुआ'));
        loadOrderById(selectedOrder._id);
      } else {
        setMessage(res.message || uiT('Failed to collect payment', 'भुगतान एकत्र करने में विफल'));
      ...
      if (res.success) {
        ...
        globalToast.success(
          uiT('Order delivered — notification sent', 'ऑर्डर डिलीवर हुआ — सूचना भेजी गई')
        );
        fetchReadyOrders();
        ...
        setMessage(res.message || uiT('Failed to deliver order', 'ऑर्डर डिलीवर करने में विफल'));
        globalToast.error(res.message || uiT('Failed to deliver', 'डिलीवर करने में विफल'));
      }
    }
    setDelivering(false);
  ...
  function orderItemsLabel(o: Order): string {
    return (
      [(o as any).frame, o.lensBrand, o.coating].filter(Boolean).join(' + ') ||
      uiT('Order items', 'ऑर्डर आइटम')
    );
  }
  ...
  const renderReadyCard = (o: Order) => {
    ...
    return (
      <ShineCard
        key={o._id}
        onClick={() => pickReadyOrder(o)}
        role="button"
        aria-label={uiT('Select order for', 'ऑर्डर चुनें') + ' ' + (cName || '')}
        className="bg-th-elevated rounded-xl p-3.5 cursor-pointer hover:bg-th-card transition-all"
      >
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-[#1ed760]/10 rounded-full flex items-center justify-center text-[#1ed760] font-bold text-sm flex-shrink-0">
            {(cName?.charAt(0) || '?').toUpperCase()}
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <p className="text-sm font-semibold text-th-text truncate">{cName || '—'}</p>
              <span className="text-[11px] font-bold px-1.5 py-0.5 rounded bg-[#1ed760]/10 text-[#1ed760] uppercase tracking-wider flex-shrink-0">
                {uiT('Ready', 'तैयार')}
              </span>
            </div>
            <p className="text-xs text-th-secondary mt-0.5 truncate">
              {[(o as any).frameBrand, o.lensBrand, (o as any).coating]
                .filter(Boolean)
                .join(' + ') || orderItemsLabel(o)}
            </p>
            {cMobile && <p className="text-[11px] text-th-muted mt-0.5">{cMobile}</p>}
          </div>
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 bg-[#1ed760]/10 rounded-full flex items-center justify-center text-[#1ed760] font-bold text-sm flex-shrink-0">
        ...
        <ChevronRight size={18} className="text-th-muted flex-shrink-0" aria-hidden="true" />
      </div>
    </ShineCard>
  );
    ...
    <div className="bg-th-base min-h-screen">
      <div className="max-w-[1800px] mx-auto px-3 sm:px-6 lg:px-8 py-5 md:py-7 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 bg-[#1ed760] rounded-full flex items-center justify-center text-black flex-shrink-0">
              <Package size={22} aria-hidden="true" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-th-text">
                {uiT('Pickup', 'पिकअप')}
              </h1>
              <p className="text-sm text-th-secondary">
                {uiT(
                  'Collect ready orders and manage deliveries.',
                  'तैयार ऑर्डर एकत्र करें और डिलीवरी प्रबंधित करें।'
                )}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[400px_minmax(0,1fr)] gap-5 items-start">
          ...
          <div className="space-y-4 xl:sticky xl:top-6">
            ...
            <div className="bg-th-surface rounded-2xl shadow-lg p-4">
              ...
              {message && !message.includes('✓') && !selectedOrder && (
            ...
            {customers.length > 0 && !selectedCustomer && (
              <div className="bg-th-surface rounded-2xl shadow-lg p-4 space-y-2">
                <p className="text-xs font-medium text-th-secondary uppercase tracking-wider">
                  {customers.length} {uiT('customer(s) found', 'ग्राहक मिले')}
                </p>
            ...
            {/* Selected customer */}
            {selectedCustomer && (
              <div className="bg-th-surface rounded-2xl shadow-lg overflow-hidden">
                <div className="p-4 border-b border-th-border">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-th-text truncate">
                    ...
                    <button
                      ...
                      className="text-sm text-th-secondary hover:text-th-text flex-shrink-0"
                    >
                      {uiT('Change', 'बदलें')}
                    </button>
                  </div>
                        ...
                        <div
                          ...
                          className={`flex items-center justify-between p-3.5 rounded-xl cursor-pointer transition-all ${
                            selectedOrder?._id === o._id
                              ? 'bg-[#1ed760]/10 ring-1 ring-[#1ed760]/40'
                        ...
                        >
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-th-secondary mt-0.5">
                              {o.deliveryDate &&
                                `Expected: ${new Date(o.deliveryDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}`}
                            </p>
                          </div>
          ...
          <div className="space-y-4">
            {!selectedOrder ? (
              <div className="bg-th-surface rounded-2xl shadow-lg flex flex-col items-center justify-center py-16 px-6 text-center">
                ...
                <p className="text-sm text-th-secondary mt-1 max-w-sm">
                  {uiT(
                    'Search a customer or pick a ready order to start pickup.',
                    'शुरू करने के लिए ग्राहक खोजें या तैयार ऑर्डर चुनें।'
                  )}
                </p>
              </div>
            ) : (
              <div className="bg-th-surface rounded-2xl shadow-lg p-5 space-y-5">
                {/* Order header */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-11 h-11 bg-[#1ed760]/10 rounded-full flex items-center justify-center text-[#1ed760] font-bold text-base flex-shrink-0">
                      {selectedCustomer?.name?.charAt(0)?.toUpperCase() || '?'}
                    </div>
                    <div className="min-w-0">
                      <p className="text-[17px] font-bold text-th-text truncate">
                        {selectedCustomer?.name || '—'}
                      </p>
                      <p className="text-sm text-th-secondary">{selectedCustomer?.mobile}</p>
                    </div>
                  ...
                  <button
                    ...
                    className="text-xs text-[#1ed760] hover:text-[#1ed760]/80 flex-shrink-0"
                  >
                    {uiT('Change', 'बदलें')}
                  </button>
                </div>
                ...
                {/* Order items */}
                <div>
                  <h3 className="text-sm font-semibold text-th-text flex items-center gap-1.5 mb-2">
                    <Package size={15} className="text-[#1ed760]" aria-hidden="true" />{' '}
                    {uiT('Order Details', 'ऑर्डर विवरण')}
                  </h3>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    {(selectedOrder as any).frame && (
                      <div className="bg-th-elevated rounded-lg px-4 py-3">
                        <p className="text-[14px] text-[#1ed760] uppercase tracking-wider mb-0.5 flex items-center gap-1">
                          <Glasses size={11} aria-hidden="true" /> {uiT('Frame', 'फ्रेम')}
                        </p>
                        <p className="text-sm font-medium text-th-text">
                          {selectedOrder.frameBrand
                            ? `${selectedOrder.frameBrand} ${selectedOrder.frameModel || ''}`.trim()
                            : (selectedOrder as any).frame}
                        </p>
                        {(selectedOrder as any).framePrice > 0 && (
                          <p className="text-xs text-th-secondary mt-0.5">
                            ₹{(selectedOrder as any).framePrice}
                          </p>
                        )}
                      </div>
                    )}
                    {selectedOrder.lensBrand && (
                      <div className="bg-th-elevated rounded-lg px-4 py-3">
                        <p className="text-[14px] text-th-secondary uppercase tracking-wider mb-0.5 flex items-center gap-1">
                          <Eye size={11} aria-hidden="true" /> {uiT('Lens', 'लेंस')}
                        </p>
                        <p className="text-sm font-medium text-th-text">
                          {selectedOrder.lensBrand}
                          {selectedOrder.lensType ? ` · ${selectedOrder.lensType}` : ''}
                        </p>
                        {(selectedOrder as any).lensPrice > 0 && (
                          <p className="text-xs text-th-secondary mt-0.5">
                            ₹{(selectedOrder as any).lensPrice}
                          </p>
                        )}
                      </div>
                    )}
                    {selectedOrder.deliveryDate && (
                      <div className="bg-th-elevated rounded-lg px-4 py-3">
                        <p className="text-[14px] text-th-secondary uppercase tracking-wider mb-0.5">
                          {uiT('Delivery Date', 'डिलीवरी तिथि')}
                        </p>
                ...
                {bill ? (
                  <div className="bg-th-elevated rounded-lg p-4 space-y-2">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-semibold text-th-text flex items-center gap-1.5">
                        <Receipt size={15} className="text-[#1ed760]" aria-hidden="true" />{' '}
                        {uiT('Bill Summary', 'बिल सारांश')}
                      </h3>
                      <span className="text-xs font-medium text-[#1ed760] bg-[#1ed760]/10 px-2.5 py-0.5 rounded-lg">
                        {bill.billNumber}
                      </span>
                    </div>
                    ...
                    {bill.advancePaid > 0 && (
                      <div className="flex justify-between text-sm text-[#1ed760] font-medium">
                        <span>{uiT('Advance Paid', 'अग्रिम भुगतान')}</span>
                        <span>₹{bill.advancePaid?.toFixed(0)}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-base font-bold text-[#e74c3c]">
                      <span>{uiT('Pending', 'बाकी')}</span>
                      <span>₹{bill.pendingAmount?.toFixed(0) || '0'}</span>
                    </div>
                  ...
                  <div className="bg-th-elevated rounded-lg p-4 space-y-3">
                    <h3 className="text-sm font-semibold text-th-text flex items-center gap-1.5">
                      <FileText size={15} className="text-th-secondary" aria-hidden="true" />{' '}
                      {uiT('Create Bill', 'बिल बनाएं')}
                    </h3>
                    <p className="text-xs text-th-secondary">
                      {uiT(
                        'Items auto-filled from order. Adjust as needed.',
                        'आइटम ऑर्डर से स्वतः भरे गए। आवश्यकतानुसार समायोजित करें।'
                      )}
                    </p>
                    <div className="space-y-2">
                      {billItems.map((item, idx) => (
                        <div
                          key={idx}
                          className="flex gap-2 items-start bg-th-elevated p-3 rounded-lg"
                        >
                          <div className="flex-1">
                            <input
                              className="input-field text-sm"
                              placeholder={uiT('Item description', 'आइटम विवरण')}
                              aria-label={uiT('Item description', 'आइटम विवरण')}
                          ...
                          <div className="w-16 text-right pt-2.5 text-sm font-medium text-th-secondary">
                            ₹{(item.qty * item.price).toFixed(0)}
                          </div>
                          <button
                            onClick={() => removeBillItem(idx)}
                            aria-label={uiT('Remove bill item', 'बिल आइटम हटाएं')}
                            className="p-2 hover:bg-[#e74c3c]/10 rounded-lg text-[#e74c3c]"
                          >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-32">
                        ...
                        <input
                          ...
                          min="0"
                          className="input-field text-sm"
                          aria-label={uiT('Discount amount', 'छूट राशि')}
                          value={billDiscount}
                      ...
                      <div className="flex-1 text-right">
                        <p className="text-xs text-th-secondary">
                          ...
                          <span className="font-medium text-th-text">
                            ₹{billSubtotal.toFixed(0)}
                          </span>
                        </p>
                        <p className="text-lg font-bold text-th-text">
                          {uiT('Total', 'कुल')}: ₹{billTotal.toFixed(0)}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-2 pt-1">
                      <button
                        ...
                        aria-label={uiT('Create bill', 'बिल बनाएं')}
                        className="btn-primary flex items-center gap-2 px-6 py-3"
                      >
                        <Plus size={18} aria-hidden="true" /> {uiT('Create Bill', 'बिल बनाएं')} — ₹
                        {billTotal.toFixed(0)}
                      </button>
                      <button
                        onClick={() => setShowCreateBill(false)}
                        aria-label={uiT('Cancel bill creation', 'बिल निर्माण रद्द करें')}
                  ...
                  <div className="text-center py-6 space-y-3 bg-th-elevated rounded-lg">
                    ...
                    <div>
                      <p className="text-sm font-medium text-th-secondary">
                        {uiT('No bill found for this order', 'इस ऑर्डर के लिए कोई बिल नहीं मिला')}
                      </p>
                      <p className="text-xs text-th-muted mt-0.5">
                        {uiT(
                          'Create a bill from the order items to proceed.',
                          'आगे बढ़ने के लिए ऑर्डर आइटम से बिल बनाएं।'
                        )}
                      </p>
                    </div>
                    <button
                      onClick={() => setShowCreateBill(true)}
                      aria-label={uiT('Create bill from order', 'ऑर्डर से बिल बनाएं')}
                      className="btn-primary btn-sm flex items-center gap-1 mx-auto"
                    >
                ...
                {!bill && (
                  <p className="text-xs text-amber-500 text-center">
                    {uiT(
                      'Create a bill first to mark as delivered',
                      'डिलीवर चिन्हित करने के लिए पहले बिल बनाएं'
                    )}
                  </p>
                )}
              </div>
      ...
      {showConfirmDeliver && (
        <div
          ...
          onClick={() => setShowConfirmDeliver(false)}
        >
          <div
            className="bg-th-surface rounded-lg p-6 max-w-sm w-full mx-4 shadow-lg animate-scale-in"
          ...
          >
            ...
            {(selectedOrder as any)?.status === 'Delivered' ? (
              <>
                <h3 className="text-lg font-bold text-th-text text-center mb-2">
                  {uiT('Collect Payment', 'भुगतान एकत्र करें')}
                </h3>
              ...
              <>
                <h3 className="text-lg font-bold text-th-text text-center mb-2">
                  {uiT('Confirm Delivery', 'डिलीवरी की पुष्टि करें')}
                </h3>
                <p className="text-sm text-th-secondary text-center mb-5">
                  {collectAmount > 0
              ...
              >
                ...
                {(selectedOrder as any)?.status === 'Delivered'
                  ? delivering
                    ...
                    : uiT('Confirm Collection', 'एकत्रीकरण की पुष्टि करें')
                  : delivering
                    ? uiT('Delivering...', 'डिलीवर हो रहा है...')
                    : uiT('Confirm Delivery', 'डिलीवरी की पुष्टि करें')}
              </button>
              <button
                onClick={() => setShowConfirmDeliver(false)}
                ...
                className="w-full bg-th-elevated hover:bg-th-hover text-th-text font-bold uppercase tracking-wider text-xs rounded-lg py-3.5 transition-all"
              >
                {uiT('Cancel', 'रद्द करें')}
              </button>
            </div>
