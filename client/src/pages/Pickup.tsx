...
import { Search, Phone, Check, ChevronRight, Plus, Loader2, Package, Clock, X, User, FileText, Receipt, Glasses, Eye, FlaskConical, Circle } from "lucide-react";
...
export default function Pickup() {
  ...
  function orderItemsLabel(o: Order): string {
    return [(o as any).frame, o.lensBrand, o.coating].filter(Boolean).join(" + ") || uiT("Order items", "ऑर्डर आइटम");
  }
  ...
  const renderReadyCard = (o: Order) => {
    ...
    return (
      <ShineCard key={o._id} onClick={() => pickReadyOrder(o)}
        role="button"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold px-1.5 py-0.5 rounded bg-[#1ed760]/10 text-[#1ed760] uppercase tracking-wider flex-shrink-0">{uiT("Ready", "तैयार")}</span>
            </div>
            <p className="text-xs text-th-secondary mt-0.5 truncate">
              {[(o as any).frameBrand, o.lensBrand, (o as any).coating].filter(Boolean).join(" + ") || orderItemsLabel(o)}
            </p>
            {cMobile && <p className="text-[11px] text-th-muted mt-0.5">{cMobile}</p>}
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            {(o.billInfo?.totalAmount ?? 0) > 0 && (
              <span className="text-sm font-bold text-th-text">₹{(o.billInfo?.totalAmount ?? 0).toLocaleString()}</span>
            )}
      ...
      <div className="max-w-[1800px] mx-auto px-3 sm:px-6 lg:px-8 py-5 md:py-7 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-th-text">{uiT("Pickup", "पिकअप")}</h1>
          ...
          <div className="space-y-4 xl:sticky xl:top-6">
            ...
            <div className="bg-th-surface rounded-2xl shadow-lg p-4">
              <div className="flex gap-2">
                ...
                <button onClick={searchCustomer} disabled={isLoading} aria-label={uiT("Search customer", "ग्राहक खोजें")} className="btn-primary px-6 py-3 flex-shrink-0">
                ...
                <div className="p-4 border-b border-th-border">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-th-text truncate">{selectedCustomer.name}</p>
                    ...
                    <button onClick={() => { setSelectedCustomer(null); setCustomers([]); setOrders([]); setSelectedOrder(null); setBill(null); }}
                      ...
                      className="text-sm text-th-secondary hover:text-th-text flex-shrink-0">{uiT("Change", "बदलें")}</button>
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      {orders.map((o) => (
                        <div key={o._id} onClick={() => selectOrder(o)}
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-medium text-th-text truncate">{orderItemsLabel(o)}</p>
          ...
          <div className="space-y-4">
            {!selectedOrder ? (
              <div className="bg-th-surface rounded-2xl shadow-lg flex flex-col items-center justify-center py-28 px-6 text-center">
                ...
                <p className="text-lg font-bold text-th-text">{uiT("No order selected", "कोई ऑर्डर चुना नहीं गया")}</p>
                <p className="text-sm text-th-secondary mt-1 max-w-sm">
                  {uiT("Search a customer or pick a ready order to start pickup.", "शुरू करने के लिए ग्राहक खोजें या तैयार ऑर्डर चुनें।")}
                </p>
              </div>
            ) : (
              <div className="bg-th-surface rounded-2xl shadow-lg p-5 space-y-5">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-[17px] font-bold text-th-text truncate">{selectedCustomer?.name || "—"}</p>
                      <p className="text-sm text-th-secondary">{selectedCustomer?.mobile}</p>
                    </div>
                  </div>
                  <button onClick={() => { setSelectedOrder(null); setBill(null); }}
                    aria-label={uiT("Change order", "ऑर्डर बदलें")}
                    className="text-xs text-[#1ed760] hover:text-[#1ed760]/80 flex-shrink-0">{uiT("Change", "बदलें")}</button>
                </div>
