...
export default function InventoryV2() {
  ...
  return (
    <div className="space-y-6">
      ...
      {active === "overview" && (
        <OverviewView onAction={openAction} onStartCount={startCount} onShowStock={() => setActive("stock")} />
      )}
      {active === "stock" && (
        <StockView onAction={openAction} refreshKey={refreshKey} onRefresh={refresh} onNewItem={() => openAction({ type: "new" })} />
      )}
      {active === "history" && <HistoryView refreshKey={refreshKey} />}
      {active === "manage" && (
        <ManageView refreshKey={refreshKey} openCountCreate={openCountCreate} onCountCreateHandled={() => setOpenCountCreate(false)} />
      )}

      <AddStockModal open={action?.type === "add"} variant={action?.variant} onClose={closeAction} onDone={handleDone} />
