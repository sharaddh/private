...
export default function Reports() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (cust.success) {
      const list =
        (cust.data as any)?.data || ((Array.isArray(cust.data) ? cust.data : []) as Customer[]);
      setCustomerData(list);
    }
    if (bills.success) {
      const billList = Array.isArray(bills.data)
        ? (bills.data as Bill[])
        : (bills.data as any)?.data || ([] as Bill[]);
  ...
  function exportCSV() {
    ...
    const headers = [
      ...
      uiT('Mobile', 'मोबाइल'),
      uiT('Visits', 'विज़िट'),
      uiT('Total Spent', 'कुल खर्च'),
      uiT('Pending Amount', 'बाकी राशि'),
    ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="page-subtitle">
            {uiT('Business insights and analytics.', 'व्यापार जानकारी और विश्लेषण।')}
          </p>
        </div>
        <div className="flex gap-2">
          {activeTab === 'customers' && customerData?.length > 0 && (
            <button
              onClick={exportCSV}
              aria-label={uiT('Export customers CSV', 'ग्राहक CSV निर्यात')}
              className="btn-secondary btn-sm flex items-center gap-1.5"
            >
              <Download size={14} /> {uiT('Export CSV', 'CSV निर्यात')}
            </button>
          )}
          {activeTab === 'pending' && pendingData?.length > 0 && (
            <button
              onClick={exportPendingCSV}
              aria-label={uiT('Export pending bills CSV', 'बाकी बिल CSV निर्यात')}
              className="btn-secondary btn-sm flex items-center gap-1.5"
            >
              <Download size={14} /> {uiT('Export CSV', 'CSV निर्यात')}
            </button>
      ...
      {(activeTab === 'sales' || activeTab === 'pending') && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {DATE_PRESETS.map((p, i) => (
            ...
            >
              {uiT(p.label, presetLabels[p.label] || p.label)}
            </button>
          ))}
          ...
          {(customStart || customEnd) && (
            <button
              ...
              className="text-xs text-th-muted hover:text-[#e74c3c] px-2"
            >
              Clear
            </button>
          )}
      ...
      {activeTab === 'customers' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-th-text">
                {(customerData?.length || 0).toLocaleString()}
              </p>
            ...
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-[#1ed760]">
                {(
                  customerData?.filter((c: Customer) => c.totalVisits === 1).length || 0
                ).toLocaleString()}
              </p>
            ...
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-[#1ed760]">
                {(
                  customerData?.filter((c: Customer) => (c.totalVisits || 0) > 1).length || 0
                ).toLocaleString()}
              </p>
              <p className="text-sm text-th-secondary">
                {uiT('Returning Customers', 'वापसी ग्राहक')}
              </p>
            </ShineCard>
          </div>
          ...
          {topCustomers.length > 0 && (
            <div className="card">
              <h3 className="section-title mb-4">
                {uiT('Top Customers by Spend', 'खर्च के अनुसार शीर्ष ग्राहक')}
              </h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-th-border">
                      ...
                      <th className="text-left py-2 px-3 text-th-muted font-medium">
                        {uiT('Name', 'नाम')}
                      </th>
                      <th className="text-right py-2 px-3 text-th-muted font-medium">
                        {uiT('Visits', 'विज़िट')}
                      </th>
                      <th className="text-right py-2 px-3 text-th-muted font-medium">
                        {uiT('Total Spent', 'कुल खर्च')}
                  ...
                  <tbody>
                    {topCustomers.slice(0, 10).map((c: Customer, i: number) => (
                      <tr key={c._id} className="border-b border-th-card hover:bg-th-elevated">
                        ...
                        <td className="py-2 px-3 text-right text-th-secondary">
                          {c.totalVisits || 0}
                        </td>
                        <td className="py-2 px-3 text-right text-[#1ed760] font-medium">
          ...
          <div className="card">
            <h3 className="section-title mb-4">
              {uiT('All Customers', 'सभी ग्राहक')} ({customerData?.length || 0})
            </h3>
            {!customerData || customerData.length === 0 ? (
              <p className="text-th-muted text-sm text-center py-8">
      ...
      {activeTab === 'sales' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <ShineCard className="card text-center">
              ...
              <p className="text-xs text-th-secondary">
                {uiT('Total', 'कुल')} {uiT('Revenue', 'राजस्व')}
              </p>
            </ShineCard>
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-[#1ed760]">
                ₹{(salesData?.totalCollection || 0).toLocaleString()}
              </p>
            ...
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-amber-400">
                ₹
                {(
                  (salesData?.totalRevenue || 0) - (salesData?.totalCollection || 0)
                ).toLocaleString()}
              </p>
            ...
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-[#1ed760]">
                {salesData && salesData.totalRevenue > 0
                  ? `${((salesData.totalCollection / salesData.totalRevenue) * 100).toFixed(1)}%`
                  : '—'}
              </p>
              <p className="text-xs text-th-secondary">
                {uiT('Collection Ratio', 'संग्रह अनुपात')}
              </p>
            </ShineCard>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="card">
              <p className="text-xs text-th-secondary">
                {uiT('Avg Order Value', 'औसत ऑर्डर मूल्य')}
              </p>
              <p className="text-xl font-bold text-[#1ed760]">
                ₹
                {salesData && salesData.billCount > 0
                  ? (salesData.totalRevenue / salesData.billCount).toLocaleString(undefined, {
            ...
            <div className="card">
              ...
              <p className="text-xl font-bold text-[#e74c3c]">
                ₹{(salesData?.totalDiscount || 0).toLocaleString()}
              </p>
            </div>
          </div>

          {salesData?.billCount === 0 && (
            <ShineCard className="card text-center py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-[#e74c3c]">
                ₹
                {pendingData
                  .reduce((s: number, b: Bill) => s + (b.pendingAmount || 0), 0)
                  .toLocaleString()}
              </p>
              <p className="text-sm text-th-secondary">
                {uiT('Total Pending Amount', 'कुल बाकी राशि')}
              </p>
            </ShineCard>
            <ShineCard className="card text-center">
              <p className="text-3xl font-bold text-[#1ed760]">
              ...
              <p className="text-sm text-th-secondary">
                {uiT('Over 30 Days (Amount)', '30 दिन से अधिक (राशि)')}
              </p>
            </ShineCard>
          </div>
            ...
            {pendingData.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-th-muted">
                  {uiT('No pending payments.', 'कोई बाकी भुगतान नहीं।')}
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto max-h-96 overflow-y-auto">
                    ...
                    {pendingData.map((b: Bill) => {
                      ...
                      return (
                        <tr key={b._id} className="border-b border-th-card hover:bg-th-elevated">
                          ...
                          <td className="py-2 px-3 text-right">
                            <span
                              className={`font-medium ${days > 30 ? 'text-[#e74c3c]' : days > 7 ? 'text-amber-400' : 'text-th-muted'}`}
                            >
                              {days}d
                            </span>
                          </td>
      ...
      {activeTab === 'inventory' && (
        <div className="space-y-6">
          ...
          {invData?.byCategory && invData.byCategory.length > 0 && (
            <div className="card">
              <h3 className="section-title mb-4">
                {uiT('Stock by Category', 'श्रेणी अनुसार स्टॉक')}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {invData.byCategory.map((cat: { _id: string; count: number; totalQty: number }) => (
                  <div
                    key={cat._id}
                    className="flex items-center justify-between p-3 rounded-lg bg-th-elevated"
                  >
                    <div>
                      <p className="text-sm font-medium text-th-text capitalize">
                        {cat._id || uiT('Uncategorized', 'बिना श्रेणी')}
                      </p>
                      <p className="text-xs text-th-secondary">
                        {cat.count} {uiT('items', 'आइटम')}
                      </p>
                    </div>
                    <span className="text-sm font-semibold text-[#1ed760]">
                      {cat.totalQty} {uiT('units', 'इकाई')}
                    </span>
                  </div>
                ))}
              </div>
          ...
          {invData?.lowStock && invData.lowStock.length > 0 && (
            <div className="card border border-[#e74c3c]/30">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="section-title text-[#e74c3c]">
                  {uiT('Low Stock Alert', 'कम स्टॉक अलर्ट')}
                </h3>
              </div>
              <div className="space-y-2">
                {invData.lowStock.map((item: InventoryItem) => (
                  <div
                    key={item._id}
                    className="flex items-center justify-between p-3 rounded-lg bg-[#e74c3c]/5"
                  >
                    <div>
          ...
          {(!invData?.lowStock || invData.lowStock.length === 0) && (
            <ShineCard className="card text-center py-8">
              ...
              <p className="text-sm text-th-muted">
                {uiT('No inventory data available.', 'कोई इन्वेंट्री डेटा उपलब्ध नहीं।')}
              </p>
            </ShineCard>
          )}
        </div>
