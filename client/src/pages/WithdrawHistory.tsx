      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-500 mt-1">
            {uiT(
              'Record of stock withdrawn from inventory.',
              'इन्वेंट्री से निकाले गए स्टॉक का रिकॉर्ड।'
            )}
          </p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-th-hover bg-th-base">
                <th className="px-4 py-3 text-left text-[15px] font-semibold text-th-secondary uppercase tracking-wider">
                  {uiT('Date', 'दिनांक')}
                </th>
                <th className="px-4 py-3 text-left text-[15px] font-semibold text-th-secondary uppercase tracking-wider">
                  {uiT('By', 'द्वारा')}
              ...
              {list.map((w) => (
                <tr key={w._id} className="hover:bg-th-card transition-colors">
                    ...
                    <div className="flex flex-col gap-0.5">
                      {w.items.slice(0, 3).map((it) => (
                        <span key={it.sku} className="text-xs text-th-secondary whitespace-nowrap">
                          <span className="font-mono text-th-text">{it.sku}</span> — {it.brand}{' '}
                          {it.model}
                      ...
                      {w.items.length > 3 && (
                        <span className="text-xs text-th-muted">
                          +{w.items.length - 3} {uiT('more', 'और')}
                        </span>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-semibold text-th-text">
                    {w.totalQty}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-semibold text-th-text">
                    ₹{w.totalPrice}
                  ...
                  <td className="px-4 py-3 text-sm text-th-secondary max-w-[220px]">
                    <span className="block truncate" title={w.note || ''}>
                      {w.note || '—'}
                    </span>
                  </td>
                </tr>
