...
function drawTotalsBox(doc: jsPDF, bill: PdfBill, startY: number): number {
  ...
  rows.push({
    label: 'Balance Due',
    value: fmt(bill.pendingAmount || 0),
    color: warning,
    bold: true,
  });
...
function drawTermsAndThanks(
  ...
  doc.setFontSize(10);
  setText(doc, primary);
  doc.text(`Thank you for choosing ${settings.shopName || 'us'}!`, PAGE_W / 2, y, {
    align: 'center',
  });
}
...
function stampFooters(doc: jsPDF, settings: PdfSettings) {
  ...
  const footerParts = [settings.shopAddress, settings.shopPhone, settings.shopEmail].filter(
    Boolean
  );
...
export function generateThermalReceipt(
  ...
  if (items.length === 0) {
    ...
    items.forEach((it, i) => {
      ...
      lines.push(`  ${i + 1}. ${desc}`);
      lines.push(
        `     Qty: ${qty}  x  ${CURRENCY}${price.toFixed(2)}  =  ${CURRENCY}${total.toFixed(2)}`
      );
    });
  }
...
function drawRxRow(
  doc: jsPDF,
  label: string,
  data: { sph?: number; cyl?: number; axis?: number; va?: string } | undefined,
  x: number,
  y: number
): number {
  doc.setFont('helvetica', 'bold');
...
function drawRxEyeSection(
  doc: jsPDF,
  title: string,
  color: RGB,
  eye: PdfRxEye | undefined,
  x: number,
  y: number
): number {
...
export function generatePrescriptionPdf(
  ...
  if (settings.shopAddress) {
    doc.text(settings.shopAddress, rightX, infoY, { align: 'right' });
    infoY += 4;
  }
  if (settings.shopPhone) {
    doc.text(`Phone: ${settings.shopPhone}`, rightX, infoY, { align: 'right' });
  }
  ...
  y += 5;
  doc.text(`Generated on ${new Date().toLocaleString('en-IN')}`, PAGE_W / 2, y, {
    align: 'center',
  });
