...
class InventoryService extends ApiService {
  ...
  async checkSkuExists(
    sku: string
  ): Promise<ApiResponse<{ exists: boolean; item?: InventoryItem }>> {
    return api.get<{ exists: boolean; item?: InventoryItem }>(
      `${this.basePath}/exists?sku=${encodeURIComponent(sku)}`
    );
  }
  ...
  async adjustStock(
    ...
    return api.put<InventoryItem>(`${this.basePath}/${id}/stock`, {
  ...
  async importItems(
    ...
    note?: string
  ): Promise<ApiResponse<InventoryImportResult>> {
