...
class OrderService extends ApiService {
  ...
  async listFiltered(
    params: PaginationParams & DateRangeParams & { status?: string; dateField?: string }
  ...
  ): Promise<ApiResponse<Order & { partial?: boolean }>> {
    return api.patch<Order & { partial?: boolean }>(`${this.basePath}/${id}/status`, {
      status,
      advanceQuantity,
    });
  }
