  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        ...
        if (res.success) {
          toast('Lens updated successfully');
          onSaved();
          onClose();
        } else {
        ...
        if (res.success) {
          toast('Lens created successfully');
          onSaved();
          onClose();
        } else {
  ...
  return (
    <Modal open={open} onClose={onClose} title={editing ? 'Edit Lens' : 'Add Lens'} size="lg">
      <form onSubmit={handleSave} className="p-6 space-y-4">
        {error && (
          <div className="bg-negative/10 text-negative px-4 py-3 rounded-pill text-sm">{error}</div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              value={form.location}
              onChange={(e) =>
                setForm({ ...form, location: e.target.value as 'shop' | 'warehouse' })
              }
            >
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {saving ? (
              <>
                <Spinner size={14} className="border-surface-950 border-t-transparent" /> Saving...
              </>
            ) : editing ? (
              'Update Lens'
            ) : (
