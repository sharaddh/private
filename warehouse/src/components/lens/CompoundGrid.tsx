...
const SphCell = memo(function SphCell({
  sph,
  cyl,
  qty,
  onIncrement,
  ...
  return (
    <div className={`flex flex-col items-center gap-2.5 p-3 rounded-xl border ${border} ${bg}`}>
      <span className="text-sm sm:text-base font-bold leading-none whitespace-nowrap">
        <span
          className={isNeg ? 'text-amber-500' : isPos ? 'text-emerald-500' : 'text-th-secondary'}
        >
          {sphLabel}
        </span>
        <span className="text-th-muted"> | </span>
        <span className={cylNeg ? 'text-amber-500' : cylPos ? 'text-emerald-500' : 'text-th-muted'}>
          {cylLabel}
        </span>
      </span>
