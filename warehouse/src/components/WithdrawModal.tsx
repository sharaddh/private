...
export default function WithdrawModal({ item, onClose, onSaved }: Props) {
  ...
  async function handleWithdraw() {
    ...
    setWithdrawing(true);
    const res = await api.put('/api/warehouse/inventory/' + item._id + '/stock', {
      quantity: -qty,
    });
    if (res.success) {
      toast('Stock withdrawn successfully');
      onSaved();
      onClose();
    } else {
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={withdrawing || qty <= 0 || qty > (item.quantity || 0)}
            className="btn-danger flex-1 flex items-center justify-center gap-2"
          >
            {withdrawing ? (
              <>
                <Spinner size={14} className="border-white border-t-transparent" /> Processing...
              </>
            ) : (
              'Withdraw'
            )}
