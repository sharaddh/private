        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="page-subtitle">
              {marks.length} mark{marks.length !== 1 ? 's' : ''} available to select in cart
            </p>
          </div>
        </div>
        <button
          onClick={openAdd}
          className="btn-primary flex items-center gap-2 shrink-0 px-4 sm:px-6"
        >
        ...
        <div className="card overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody>
                {marks.map((mark) => (
                  <tr
                    key={mark._id}
                    className="border-b border-th-border hover:bg-th-hover transition-colors"
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="p-2 hover:bg-th-hover rounded-lg text-th-muted hover:text-announcement transition-colors"
                          title="Edit"
                        >
                          <Pencil size={15} />
                        </button>
                        <button
                          ...
                          className="p-2 hover:bg-th-hover rounded-lg text-th-muted hover:text-negative transition-colors"
                          title="Delete"
                        >
                          <Trash2 size={15} />
                        </button>
          ...
          <div className="sm:hidden space-y-2 p-3">
            {marks.map((mark) => (
              <div
                key={mark._id}
                className="flex items-center justify-between gap-2 bg-th-elevated rounded-lg p-3"
              >
                <div className="min-w-0">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    onClick={() => openEdit(mark)}
                    className="w-9 h-9 rounded-lg bg-th-elevated text-th-muted flex items-center justify-center active:scale-90 transition-all hover:text-announcement"
                    title="Edit"
                  >
                    <Pencil size={15} />
                  </button>
                  <button
                    onClick={() => setDeleteTarget(mark)}
                    className="w-9 h-9 rounded-lg bg-th-elevated text-th-muted flex items-center justify-center active:scale-90 transition-all hover:text-negative"
                    title="Delete"
                  >
                    <Trash2 size={15} />
                  </button>
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-start gap-3 bg-th-elevated rounded-lg p-3">
            ...
            <p className="text-body text-th-text">
              Delete <span className="font-bold">"{deleteTarget?.name}"</span>? Existing cart items
              keep their selection but it won't appear as an option.
            </p>
          </div>
