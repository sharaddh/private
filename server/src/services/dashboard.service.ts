...
export async function getStats() {
  ...
  const [
    ...
    Bill.find({ status: "Active", pendingAmount: { $gt: 0 } })
      .sort({ createdAt: -1 })
    ...
    Order.find({ createdAt: { $gte: dayStart, $lte: dayEnd } })
      ...
      .populate("customerId", "name mobile")
      .lean(),
    Order.find({ status: "Ready", deliveryDate: { $gte: dayStart, $lte: dayEnd } })
    ...
    Bill.aggregate([
      { $match: { createdAt: { $gte: thirtyDaysAgo, $lte: dayEnd }, status: "Active" } },
      {
        $group: {
          _id: {
            $dateToString: { format: "%Y-%m-%d", date: "$createdAt", timezone: BUSINESS_TIMEZONE },
          },
    ...
    Order.find({ status: "Delivered", actualDeliveryDate: { $gte: dayStart, $lte: dayEnd } })
      .populate("customerId", "name mobile")
    ...
    Order.find({ status: { $in: ["Draft", "Ordered", "In Lab"] } })
      .sort({ createdAt: -1 })
      .populate("customerId", "name mobile")
      .lean(),
    Order.aggregate([
  ...
  const recentOrderVisitIds = recentOrders
    .map(
      (o: any) => o.visitId?._id?.toString() || (typeof o.visitId === "string" ? o.visitId : null)
    )
    .filter(Boolean);
  ...
  const allPrescriptions =
    allVisitIds.length > 0 ? await Prescription.find({ visitId: { $in: allVisitIds } }).lean() : [];
  ...
  const mappedOrderStatusCounts = orderStatusCounts.map((d: { _id: string; count: number }) => ({
    status: d._id,
    count: d.count,
  }));
