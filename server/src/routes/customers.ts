...
router.post("/", authenticate, validate(createCustomerSchema, "body"), asyncHandler(async (req, res, next) => {
...
router.put("/:id", authenticate, validate(updateCustomerSchema, "body"), asyncHandler(async (req, res) => {
  await invalidateCache("/api/customers");
  await invalidateCache("/api/dashboard");
